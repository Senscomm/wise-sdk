...
static declare_device_single(spi, 3) = {
    .name = "atcspi200-xip.2",
    .base[0] = (void *) SPI2_BASE_ADDR,
    .base[1] = (void *) 0x90000000,
};

flash_part_t usr_part = {
    .start = 0x90000000,
    .size = 0x800000,
};

#ifdef CONFIG_SPIFFS_ON_SPI2
FILESYSTEM(spiffs2) = {
    .name     = "spiffs2",
    .root     = "/ext",
    .private  = &usr_part,
    .fs_list  = LIST_HEAD_INIT(filesystem(spiffs2)->fs_list),
...
#endif /* CONFIG_SPIFFS_ON_SPI2 */
#endif /* CONFIG_USE_SPI2_FLASH */
#endif /* CONFIG_USE_SPI2 */

/*
 * C1T301A34 V3.0 b/d pin assignment
 */
...
#ifdef CONFIG_USE_SPI2
#ifdef CONFIG_USE_SPI2_FLASH
    struct clk *pclk, *mux, *clk_80m;

    pclk = clk_get(NULL, "atcspi200-xip.2/pclk");
    mux = clk_get_parent(pclk);
    clk_80m = clk_get(NULL, "80m");
    clk_set_parent(mux, clk_80m);
#endif
#endif

	pinctrl_set_platform_pinmap(&board_pin_map);

	printk("BOARD: C1T301A34 EVB\n");
}
