...
#define INFO(_jedec_id, _ext_id, _block_size, _n_blocks, _flags)	\
		.id = {							\
			((_jedec_id) >> 16) & 0xff,			\
			((_jedec_id) >> 8) & 0xff,			\
			(_jedec_id) & 0xff,				\
...
#define INFO6(_jedec_id, _ext_id, _block_size, _n_blocks, _flags)	\
		.id = {							\
			((_jedec_id) >> 16) & 0xff,			\
			((_jedec_id) >> 8) & 0xff,			\
			(_jedec_id) & 0xff,				\
...
const struct spi_flash_info spi_flash_ids[] = {
	...
	{"sst26wf080b",    INFO(0xbf2658, 0x0,  64 * 1024,    16, SECT_4K) },
#endif
