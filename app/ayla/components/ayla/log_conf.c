				...
				size_t len, struct ayla_tlv *tlv)
{
	...
	case CT_mod:
		...
		log_mods[i].mask = (enum log_mask)conf_get_u32(tlv);
		break;
	case CT_client:
...
{
	enum conf_token tk[4] = {CT_log, CT_mod, CT_none, CT_mask};
	char name[30];
	long lval;
	int i;
	int len;
	...
	for (i = LOG_MOD_DEFAULT; i < __MOD_LOG_LIMIT; i++) {
		tk[2] = (enum conf_token)i;
		len = conf_tokens_to_str(tk, ARRAY_LEN(tk), name, sizeof(name));
		ASSERT(len < sizeof(name));
		if (conf_persist_get_long(name, &lval) || lval < 0) {
			continue;
		}
		log_mods[i].mask = (enum log_mask)lval;
	}
}
