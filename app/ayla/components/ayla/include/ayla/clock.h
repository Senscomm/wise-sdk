...
enum clock_src {
	...
	CS_MIN = AL_CS_MIN,
	CS_DEF = AL_CS_DEF,		/* set to CLOCK_START */
	CS_PERSIST = AL_CS_PERSIST,	/* from persisted data store (nvram) */
	CS_HTTP = AL_CS_HTTP,		/* from cloud via HTTP request */
	CS_LOCAL = AL_CS_LOCAL,		/* set by internal web server */
	CS_MCU_LO = AL_CS_MCU_LO,	/* set by MCU. low priority */
	CS_SERVER = AL_CS_SERVER,	/* set using server time */
	CS_SNTP = AL_CS_SNTP,		/* set using SNTP, not kept in sync */
	CS_NTP = AL_CS_NTP,		/* set using NTP */
	CS_MCU_HI = AL_CS_MCU_HI,	/* set by MCU. high priority */
...
/*
 * convert time to "YYYY-MM-DDThh:mm:ss" ISO format
 */
 ...
 * This is so that the TLS certificates will appear valid.
 */
#define CLOCK_START 1640995200	/* Jan 1, 2022 00:00 UTC */
...
extern const char ayla_time_cli_help[];
