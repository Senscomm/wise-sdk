...
#ifdef __cplusplus
	void (*xport)(void);	/* export is a reserved word in C++ */
#else
	void (*export)(void);
#endif
...
struct conf_state {
	...
	u8 path_len;
	u8 name_len;
	u8 show_conf:1;		/* don't save, doing 'conf show' */
	u8 show_conf_matched:1;	/* set if a name matched during 'conf show' */
	u8 save_as_factory:1;	/* saving conf as factory config */
	u8 no_print:1;		/* do not print for show_conf */
	u8 item_found:1;	/* set if unique item found during walk */
	u8 *next;		/* conf_resp() buffer pointer */
	enum al_persist_section show_conf_section;
	size_t rlen;
...
#define CONF_OP_SS_WIFI_REGION	0x00000170	/* wifi region */
...
extern u8 conf_setup_mode_test;		/* allow enabling setup without reset */
extern u8 conf_test_mode;
...
/*
 * Locking is required on platforms which use config from more than one thread.
 */
void conf_lock(void);
 ...
 * Returns the length of the value on success or enum ada_err value on error.
 */
...
/*
 * Export config item to flash or to display (depending on config state)
 */
enum al_err conf_persist_export(const char *name, const void *buf, size_t len);

/*
 * Return non-zero if configuration matches factory configuration.
 */
