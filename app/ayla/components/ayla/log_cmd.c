		 ...
		 enum log_mask on_mask, enum log_mask off_mask)
{
	...
	mod->mask = (enum log_mask)log_mask;
	return (enum log_mask)log_mask;
}
...
const char ada_log_snap_cli_help[] = "log-snap [<n>|list|save|erase]";
...
void ada_log_snap_cli(int argc, char **argv)
{
	...
	count = log_snap_count(&max, &auto_overwrite);
	printcli("snapshots: %d saved, %d maximum, %soverwrite when full",
	   count, max, auto_overwrite ? "" : "no ");
}
#endif /* AYLA_LOG_SNAPSHOTS */

/*
 * "core" CLI.
 * This is an optional CLI handler that the host app can use if desired.
 */
const char ada_log_core_help[] = "core [dump|erase]";

void ada_log_core_cli(int argc, char **argv)
{
	if (argc == 2 && !strcmp(argv[1], "dump")) {
		if (al_log_core_dump() > 0) {
			printcli("no core dump present");
		}
		return;
	}
	if (argc == 2 && !strcmp(argv[1], "erase")) {
		al_log_core_dump_erase();
		return;
	}
	if (argc == 1) {
		if (al_log_core_dump_info() > 0) {
			printcli("no core dump present");
		}
		return;
	}
	printcli("usage: %s", ada_log_core_help);
}

const char ada_log_cli_help[] = "log [--mod <subsys>] [<level> ...]";
...
void ada_log_cli(int argc, char **argv)
{
	...
	if (log_mask_change_show(mod_name,
	    (enum log_mask)on_mask, (enum log_mask)off_mask, 1)) {
