...
/*
 * Fill the buffer with current token string.
 */
ssize_t jsmn_get_token_str(jsmn_parser *parser,
	jsmntok_t *tok, char *buf, size_t len)
{
	ssize_t rc = 0;
	...
	rc = tok->end - tok->start;
	if (rc < 0 || rc >= len) {
		return -1;
	}
	memcpy(buf, parser->js + tok->start, rc);
	buf[rc] = '\0';
	return rc;
}
...
/*
 * String comparator.
 * Returns 1 if obj is a string that matches the string arg, otherwise 0.
 */
static int jsmn_compare_string(jsmn_parser *parser, jsmntok_t *obj, void *arg)
{
	const char *str = arg;
	int str_len;
	...
	str_len = strlen(str);
	if (obj->end - obj->start != str_len ||
	    memcmp(str, parser->js + obj->start, str_len)) {
		return 0;
	}
	return 1;
}
...
/*
 * Iterate over an array to determine if it contains the named string element.
 * Returns 1 if string was found in the array, otherwise 0.
 */
int jsmn_array_contains_string(jsmn_parser *parser, jsmntok_t *array,
	const char *str)
{
	int rc;
	...
	rc = jsmn_array_iterate(parser, array, jsmn_compare_string,
	    (void *)str);
	if (rc == 1) {
		return 1;
	}
