...
/*
 * Return the LAN pointer given the associated source mask.
 * Returns NULL on invalid mask or if the LAN is not active.
 */
static struct client_lan_reg *client_lan_reg_from_mask(u8 src_mask)
{
	struct client_lan_reg *lan = client_lan_reg;
	u8 mask = NODES_LAN_BASE;

	for (lan = client_lan_reg; lan < ARRAY_END(client_lan_reg); lan++) {
		if (mask & src_mask) {
			if (lan->uri[0] == '\0') {
				return NULL;
			}
...
static void *client_lan_buf_alloc(void)
{
	return al_os_mem_alloc(CLIENT_LAN_BUF_LEN);
}
...
/*
 * Send state update to prop_mgrs.
 */
static void client_lan_state_update(struct client_lan_reg *lan,
	 enum ada_lan_state new_state)
{
	struct ada_lan_session sess;

	sess.state = new_state;
	sess.index = lan->id - 1;	/* lan 1 is index 0 */
	sess.remote_ipv4_addr = lan->host_addr;
	sess.remote_port = lan->http_client.host_port;
	prop_mgr_event(PME_LAN_STATE_CHANGE, (void *)&sess);
}
...
static int client_lan_next_step(struct client_lan_reg *lan)
{
	...
	if (state->echo_dest_mask & lan->mask) {
	...
	if (state->prop_send_cb && (state->dest_mask & lan->mask)) {
post:
...
static void client_lan_send_next(struct http_client *hc, enum ada_err err)
{
	...
	if (err == AE_OK) {
		if (!hc->client_send_data_cb && http_client_is_sending(hc)) {
			/* nothing to send */
					...
					void *buf, size_t len)
{
	...
	err = client_prop_send_done(state, 1,
	    state->prop_send_cb_arg, lan->mask, hc);
	if (err == AE_BUF) {
		return err;
	}
	lan->send_seq_no++;
...
{
	struct al_hmac_ctx *ctx = lan->sign_ctx;
	 ...
	 * See RFC 2104.
	 */
	al_hmac_sha256_init(ctx, secret, sec_len);
	al_hmac_sha256_update(ctx, seed, seed_len);
	al_hmac_sha256_final(ctx, dest_buf);

	al_hmac_sha256_init(ctx, secret, sec_len);
	al_hmac_sha256_update(ctx, dest_buf, CLIENT_LAN_KEY_LEN);
	al_hmac_sha256_update(ctx, seed, seed_len);
	al_hmac_sha256_final(ctx, dest_buf);
}
    ...
    char *random_two, char *time_two)
{
	...
	if (!lan->sign_ctx) {
		lan->sign_ctx = al_hmac_sha256_new();
		if (!lan->sign_ctx) {
			CLIENT_LOGF(LOG_ERR, "HMAC alloc failed");
			return -1;
		}
	}
	...
	if (!lan->aes_tx) {
		lan->aes_tx = al_aes_ctxt_alloc();
		if (!lan->aes_tx) {
			CLIENT_LOGF(LOG_ERR, "AES alloc failed");
			return -1;
		}
	}
	if (!lan->aes_rx) {
		lan->aes_rx = al_aes_ctxt_alloc();
		if (!lan->aes_rx) {
			CLIENT_LOGF(LOG_ERR, "AES alloc failed");
			return -1;
		}
	...
	rc = al_aes_cbc_key_set(lan->aes_tx,
	...
	client_lan_key_gen(state, lan, secret, sec_len, key,
	    app_root, "1");
	rc = al_aes_cbc_key_set(lan->aes_rx,
					...
					void *buf, size_t len)
{
	...
	client_timer_cancel(&lan->timer);
	lan->send_seq_no++;
	client_finish_echo(state, lan->mask);
	client_lan_tcp_recv_done(hc);
	return AE_OK;
}
...
static int client_lan_gen_random(struct client_lan_reg *lan)
{
	...
	al_random_fill(random_data, sizeof(random_data));
	rc = ayla_base64_encode(random_data, sizeof(random_data),
	...
	snprintf(lan->time_one, sizeof(lan->time_one),
			    ...
			    struct http_client *hc)
{
	...
	ame_len = snprintf(state->ame_buf,
	    sizeof(state->ame_buf), "{\"key_exchange\":{\"ver\":%u,"
	    "\"random_1\":\"%s\",\"time_1\":%s,\"proto\":%u,",
	    CLIENT_LAN_EXCH_VER, lan->random_one, lan->time_one,
	    CLIENT_LAN_PROTO_NUM);
	if (!lan->pubkey) {
		ame_len += snprintf(state->ame_buf + ame_len,
		    sizeof(state->ame_buf) - ame_len,
		    "\"key_id\":%u}}",
		    lcf->lanip_key_id);
	} else {
		ame_len += snprintf(state->ame_buf + ame_len,
		...
		rc = al_rsa_encrypt_pub(lan->pubkey,
		    state->lanip.lanip_random_key,
		    sizeof(state->lanip.lanip_random_key),
		    tmp_str, sizeof(tmp_str));

		if (ame_len < sizeof(state->ame_buf)) {
			tmp_len = sizeof(state->ame_buf) - ame_len - 1;
			ayla_base64_encode(tmp_str, rc,
			    state->ame_buf + ame_len, &tmp_len);
			ame_len += tmp_len;
		}
		ame_len += snprintf(state->ame_buf + ame_len,
		    sizeof(state->ame_buf) - ame_len,
	...
	const struct prop_dp_meta *dp_meta)
{
	...
	if (dp_meta) {
		tlen = strlen(dp_meta_ptr->key);
		while (dp_meta_ptr < dp_meta + PROP_MAX_DPMETA && tlen > 0) {
			/* add key */
			json_val_len = json_format_bytes(NULL, 0,
			    dp_meta_ptr->key, tlen, NULL, NULL, 0);
			if (json_val_len < 0 ||
			    json_val_len > PROP_DPMETA_KEY_LEN) {
				return -1;
			}
			/* add 2 for the quotes, 1 for colon */
			len += json_val_len + 2 + 1;

			/* add value */
			json_val_len = json_format_bytes(NULL, 0,
			    dp_meta_ptr->value,
			    strlen(dp_meta_ptr->value), NULL, NULL, 0);
			if (json_val_len < 0 ||
			    json_val_len > PROP_DPMETA_VAL_LEN) {
				return -1;
			}
			...
			dp_meta_ptr++;
			/* add 1 for comma if another entry */
			tlen = strlen(dp_meta_ptr->key);
			if (tlen > 0) {
				len += 1;
			}
		...
		/* add metadata tag */
		/* add 2 for quotes, 2 for braces, 1 for colon, 1 for comma */
		len += sizeof(meta_tag) + 2 + 2 + 1;
	}
...
static enum ada_err client_lan_get_prop_data(char *buf, u32 buf_len,
	...
	size_t value_len, struct prop_dp_meta *dp_meta,
	u32 *consumed, size_t send_offset)
{
	char tmpbuf[20];
	ssize_t len = 0;
	ssize_t totlen = 0;
	...
	enum client_lan_prop_state req_part = CLP_NAME_TAG;
	struct prop_dp_meta *dp_meta_ptr = dp_meta;
	...
	do {
		...
		switch (req_part) {
		case CLP_NAME_TAG:
			totlen = snprintf(tmpbuf, sizeof(tmpbuf),
			    "{\"name\":\"");
			req_part = CLP_NAME;
			break;
		case CLP_NAME:
		...
		case CLP_VAL:
			...
			tmpbuf_ptr = value;
			format_json = 1;
			req_part = CLP_METADATA_TAG;
			break;
		case CLP_METADATA_TAG:
			check = (dp_meta && strlen(dp_meta_ptr->key));
			totlen = snprintf(tmpbuf, sizeof(tmpbuf), "%s%s",
			    prop_type_is_str(type) ? "\"" : "",
			    check ? ",\"metadata\":{" : "}");
			req_part = check ? CLP_QUOTE : CLP_DONE;
			break;
		case CLP_QUOTE:
			totlen = snprintf(tmpbuf, sizeof(tmpbuf), "\"%s",
			    sep ? ",\"" : "");
			req_part = last ? CLP_END : CLP_METADATA_KEY;
			sep = 0;
			last = 0;
			break;
		case CLP_METADATA_KEY:
			totlen = strlen(dp_meta_ptr->key);
			tmpbuf_ptr = dp_meta_ptr->key;
			format_json = 1;
			req_part = CLP_MID;
			break;
		case CLP_MID:
			totlen = snprintf(tmpbuf, sizeof(tmpbuf), "\":\"");
			req_part = CLP_METADATA_VAL;
			break;
		case CLP_METADATA_VAL:
			totlen = strlen(dp_meta_ptr->value);
			tmpbuf_ptr = dp_meta_ptr->value;
			format_json = 1;
			dp_meta_ptr++;
			if (dp_meta_ptr < dp_meta + PROP_MAX_DPMETA &&
			    strlen(dp_meta_ptr->key)) {
				sep = 1;
			} else {
				last = 1;
			}
		...
		if (tmpbuf_ptr == tmpbuf) {
			ASSERT(totlen < sizeof(tmpbuf));
		}

		if (send_offset < totlen) {
			if (!format_json) {
				len = strlen(tmpbuf_ptr);
				ASSERT(len > send_offset);
				len -= send_offset;
				if (len > buf_len) {
					len = buf_len;
				}
				memcpy(buf, tmpbuf_ptr + send_offset, len);
				if (consumed) {
					(*consumed) += len;
				}
				send_offset += len;
				...
				len = json_format_bytes(buf, buf_len,
				    tmpbuf_ptr + send_offset,
				    strlen(tmpbuf_ptr) - send_offset,
 ...
 * Content-Length.
 *
 * Data_len passed in includes the sequence number encapsulation.
 */
static u32 client_lan_determine_payload_size_with_seq(ssize_t data_len)
{
	ssize_t size_of_encrypt = data_len;
	ssize_t payload_len = 0;
	const char enclosure[] = "{\"enc\":\"\",\"sign\":\"\"}";
	...
	payload_len += BASE64_LEN_EXPAND(AL_HASH_SHA256_SIZE);

	/* finally add the cost of the final enclosure for sending */
	payload_len += sizeof(enclosure) - 1;
	return payload_len;
}
...
static u32 client_lan_determine_payload_size(u16 seq_no, ssize_t data_len)
{
	...
	data_len += snprintf(NULL, 0, "{\"seq_no\":%d,\"data\":}", seq_no);

	return client_lan_determine_payload_size_with_seq(data_len);
}
	...
	ssize_t pkt_size)
{
	...
	rc = al_aes_cbc_encrypt(lan->aes_tx, lan->buf, lan->buf, pkt_size);
	if (rc < 0) {
		CLIENT_LOGF(LOG_WARN, "encr err %d", rc);
		return AE_INVAL_VAL;
	}

	rc = ayla_base64_encode((u8 *)lan->buf, pkt_size,
				...
				struct client_lan_reg *lan)
{
	...
	al_hmac_sha256_init(lan->sign_ctx, lan->mod_sign_key,
		...
		struct client_lan_reg *lan, struct http_client *hc)
{
	...
	u8 base64_sign[BASE64_LEN_EXPAND(AL_HASH_SHA256_SIZE) + 1];
	size_t base64_len = sizeof(base64_sign);
	ssize_t len;
	enum ada_err err;

	al_hmac_sha256_final(lan->sign_ctx, sign);
	if (ayla_base64_encode(sign, AL_HASH_SHA256_SIZE,
	    base64_sign, &base64_len)) {
		CLIENT_LOGF(LOG_WARN, "enc fail");
		return AE_INVAL_VAL;
	}
	len = snprintf(lan->buf, sizeof(lan->buf),
	    "\",\"sign\":\"%s\"}", base64_sign);
	if (len >= sizeof(lan->buf)) {
		len = sizeof(lan->buf) - 1;
	}
...
static void client_lan_send_resp_body(struct http_client *hc)
{
	...
	if (hc->req_part == 1) {
		...
		al_hmac_sha256_update(lan->sign_ctx, sendbuf, len + xlen);
		hc->req_part++;
	}
	if (hc->req_part == 2) {
		...
		while (lan->send_val_offset < len) {
			...
			al_hmac_sha256_update(lan->sign_ctx, sendptr,
			    pkt_size);
		}
		hc->req_part++;
	}
...
write_err:
	CLIENT_LOGF(LOG_DEBUG, "write err %d", err);
}
...
static void client_lan_send_data_cb(struct http_client *hc)
{
	...
	enum client_lan_send_data_req_part {
		LAN_REQ_START = 0,
		LAN_REQ_DATA,
		LAN_REQ_SIGN,
		LAN_REQ_END
	};
	enum client_lan_send_data_req_part next_part;

	ASSERT(prop);

	CLIENT_LOGF(LOG_DEBUG2, "lan %u prop \"%s\" part %d state %d %s",
	    lan->id, prop->name, hc->req_part, hc->state,
	    client_conn_state_string(lan->conn_state));

	value_len = prop_fmt(fmt_val, sizeof(fmt_val), prop->type,
	    prop->val, prop->len, &value);

	if (hc->req_part == LAN_REQ_START) {
		/* copy the initial part to be encrypted to a char buffer */
		leftover = tmpstr;
		len = snprintf(tmpstr, sizeof(tmpstr),
		    "{\"seq_no\":%d,\"data\":", lan->send_seq_no);
		ASSERT(len < sizeof(tmpstr));
	} else {
		leftover = lan->leftover;
	}
	while (hc->req_part <= LAN_REQ_DATA) {
		strncpy(sendbuf, leftover, sizeof(sendbuf) - 1);
		len = strlen(leftover);
		sendbuf[sizeof(sendbuf) - 2] = '\0';
		err = client_lan_get_prop_data(sendbuf + len,
		    sizeof(sendbuf) - len - 2,	/* save room for curly */
		    prop->name, value, prop->type, value_len, prop->dp_meta,
		    &consumed, lan->send_val_offset);
		if (err && err != AE_BUF) {
			goto write_err;
		}

		next_part = LAN_REQ_DATA;
		len = strlen(sendbuf);
		ASSERT(len < sizeof(sendbuf));
		if (len > CLIENT_LAN_BLK_SIZE) {
			len = CLIENT_LAN_BLK_SIZE;
		} else if (len < CLIENT_LAN_BLK_SIZE) {
			sendbuf[len++] = '}';
			sendbuf[len] = '\0';
			next_part = LAN_REQ_SIGN;
		}
		ASSERT(len <= CLIENT_LAN_BLK_SIZE);
		...
		strncpy(lan->leftover, sendbuf + CLIENT_LAN_BLK_SIZE,
		    sizeof(lan->leftover));
		lan->leftover[sizeof(lan->leftover) - 1] = '\0';
		leftover = lan->leftover;

		al_hmac_sha256_update(lan->sign_ctx, sendbuf, len);
	}
	if (hc->req_part == LAN_REQ_SIGN) {
		err = client_lan_send_signature(state, lan, hc);
		if (err) {
			goto write_err;
		}
		hc->req_part = LAN_REQ_END;
		if (hc->body_len != hc->sent_len) {
			CLIENT_LOGF(LOG_ERR, "len mismatch. head %zu sent %zu",
			    hc->body_len, hc->sent_len);
			err = AE_LEN;
			goto write_err;
		}
	}
				...
				int agent_echo)
{
	...
	len = client_lan_prop_data_size(prop->name, value,
	     prop->type, value_len, prop->dp_meta);
	if (len < 0) {
...
static u8 client_parse_lan_meta(jsmn_parser *parser, jsmntok_t *obj)
{
	struct prop_recvd *prop = &prop_recvd;
	...
	for (tcnt = 1; tcnt < obj->size && cnt < PROP_MAX_DPMETA; tcnt += 2) {
		if (jsmn_get_token_str(parser, obj + tcnt,
		    meta.key, sizeof(meta.key)) <= 0) {
			CLIENT_LOGF(LOG_WARN, "bad key entry");
			continue;
		}
		if (jsmn_get_string(parser, obj, meta.key,
		    meta.value, sizeof(meta.value)) <= 0) {
			CLIENT_LOGF(LOG_WARN, "bad val entry");
			continue;
		}
		prop->metadata[cnt] = meta;		/* struct copy */
		cnt++;
	}
	return cnt;
}
...
/*
 * Receive the response from a datapoint acknowledgement.
 */
static enum ada_err client_lan_ack_recv(struct http_client *hc,
					void *buf, size_t len)
{
	struct client_state *state = &client_state;
	struct client_lan_reg *lan = hc->parent;
...
/*
 * Format and return length of ACK message.
 * Returns length which would be used, whether it fits or not.
 */
static int client_lan_ack_format(void *buf, size_t len,
		u16 seq_no, struct prop_ack *ack)
{
	return snprintf(buf, len,
	    "{\"seq_no\":%d,\"data\":"
	    "{\"id\":\"%s\",\"ack_status\":%d,\"ack_message\":%ld}}",
	    seq_no,
	    ack->id,		/* no string escapes are required */
	    ack->status ? HTTP_STATUS_BAD_REQ : HTTP_STATUS_OK,
	    ack->msg);
}

static void client_lan_ack_send_cb(struct http_client *hc)
{
	struct client_state *state = &client_state;
	struct client_lan_reg *lan = hc->parent;
	struct prop *prop;
	...
	enum ada_err err;
	size_t remaining;
	enum client_send_lan_data_ack_part {
		LAN_ACK_START = 0,
		LAN_ACK_BODY,
		LAN_ACK_SIGN,
		LAN_ACK_FINAL
	};
	char buf[CLIENT_LAN_ACK_LEN];
	...
	if (hc->req_part == LAN_ACK_START) {
		err = client_lan_send_enc_str(hc, lan);
		if (err) {
			goto write_err;
		}
		hc->req_part = LAN_ACK_BODY;
	}

	if (hc->req_part == LAN_ACK_BODY) {
		json_len = client_lan_ack_format(buf, sizeof(buf),
		    lan->send_seq_no, ack);
		ASSERT(json_len < sizeof(buf));
		remaining = json_len - lan->send_val_offset;
		while (remaining) {
			len = remaining;
			if (len > CLIENT_LAN_BLK_SIZE) {
				len = CLIENT_LAN_BLK_SIZE;
			...
			err = client_lan_encr_send(state, lan, hc,
			    buf + lan->send_val_offset, len);
			if (err) {
				goto write_err;
			}
			al_hmac_sha256_update(lan->sign_ctx,
			    buf + lan->send_val_offset, len);
			lan->send_val_offset += len;
			remaining -= len;
		}
		hc->req_part = LAN_ACK_SIGN;
	}

	if (hc->req_part == LAN_ACK_SIGN) {
		err = client_lan_send_signature(state, lan, hc);
		if (err != AE_OK) {
			goto write_err;
		}
		hc->req_part = LAN_ACK_FINAL;
		http_client_send_complete(hc);
	}
	return;

write_err:
	CLIENT_LOGF(LOG_DEBUG, "write err %d part %u", err, hc->req_part);
	hc->hc_error = HC_ERR_SEND;
	if (err == AE_BUF) {
		hc->hc_error = HC_ERR_MEM;
	}
 ...
 */
enum ada_err client_lan_ack_send(struct prop *prop)
{
	struct http_client *hc;
	struct client_lan_reg *lan;
	char uri[CLIENT_LAN_URI_LEN + 40];
	int len;

	lan = client_lan_reg_from_mask(prop->ack->src);
	if (!lan) {
		return AE_NOTCONN;
	}
	lan->send_prop = prop;
	...
	hc = client_lan_req_new(lan);
	hc->client_tcp_recv_cb = client_lan_ack_recv;
	hc->client_send_data_cb = client_lan_ack_send_cb;
	hc->prop_callback = 1;

	len = client_lan_ack_format(NULL, 0, lan->send_seq_no, prop->ack);
	hc->body_len = client_lan_determine_payload_size_with_seq(len);

	len = snprintf(uri, sizeof(uri),
	    "%s/property/datapoint/ack.json", lan->uri);
	ASSERT(len < sizeof(uri));
	...
	jsmntok_t *obj, void *client_lan)
{
	...
	jsmn_get_string(parser, prop_t, "id",
	    prop->ack_id, sizeof(prop->ack_id));

	if (jsmn_get_val(parser, prop_t, "echo")) {
		if (jsmn_get_string(parser, prop_t, "echo",
		    echo, sizeof(echo)) < 0) {
			CLIENT_LOGF(LOG_WARN, "bad prop echo flag");
			return AE_INVAL_VAL;
		}
		prop->no_echo = !strcasecmp(echo, "none");
	}

	meta_t = jsmn_get_val(parser, prop_t, "metadata");
	if (client_parse_lan_meta(parser, meta_t)) {
		prop->prop.dp_meta = prop->metadata;
	}

	client_log(LOG_DEBUG
	    "%s: name=\"%s\" val=\"%s\" type=%s no_echo=%d",
	    __func__, prop->name, prop->val, type, prop->no_echo);
...
continue_sending:
	...
	if ((err != AE_OK && err != AE_BUF) || !lcf->auto_echo
	    || prop->no_echo) {
		/*
		 * Return AE_OK for prop set errors other than AE_BUF.
		 * Otherwise, a single property set failure like an out of
		 * range value will cause the caller to abort the LAN
		 * connection, which isn't desirable.
		 */
		return AE_OK;
	}
	state->echo_dest_mask = state->valid_dest_mask & ~lan->mask;
#ifdef AYLA_BATCH_ADS_UPDATES
	if (state->echo_dest_mask & NODES_ADS && client_batch_enabled()) {
		err = client_batch_add(prop->name, prop->type, prop->val,
		    prop->prop.len, 1, prop->metadata);
		if (err == 0) {
...
/*
 * Call indicating the property manager has finished sending a property.
 * The GET commands operation can complete.
 */
void client_lan_recv_done(u8 src_mask)
{
	...
	client_lock();
	lan = client_lan_reg_from_mask(src_mask);
	if (lan && lan->conn_state == CS_WAIT_GET) {
		client_finish_echo(state, lan->mask);
		hc = &lan->http_client;
		client_lan_tcp_recv_done(hc);
	}
...
static void client_lan_reset_one(struct client_lan_reg *lan)
{
	struct http_client *hc = &lan->http_client;
	...
	client_lan_buf_free_int(lan);
	al_rsa_ctxt_free(lan->pubkey);
	al_hmac_sha256_free(lan->sign_ctx);
	al_aes_ctxt_free(lan->aes_tx);
	al_aes_ctxt_free(lan->aes_rx);
	memset(lan, 0, sizeof(*lan));		/* clear entire LAN struct */
	lan->id = (lan - client_lan_reg) + 1;	/* id 0 is for ADS */
	lan->mask = NODES_LAN_BASE << (lan->id - 1);
	hc->parent = lan;
}
...
static void client_lan_clearout(struct client_lan_reg *lan)
{
	...
	client_lan_reset_one(lan);
	client_dest_down(lan->mask);
}
	...
	struct client_lan_reg *lan)
{
	...
	if (lan->recv_decrypted) {
		if (lan->recv_cmds) {
			iter_rc = client_parse_lan_cmd(NULL, NULL, lan);
		} else {
			iter_rc = client_parse_lan_prop(NULL, NULL, lan);
		}
		parse_err = (enum ada_err)iter_rc;
...
iterate_failed:
	...
	if (ayla_base64_decode(lan->buf, strlen(lan->buf),
	    dec_sign, &sign_len)) {
decode_failure:
	...
	if (ayla_base64_decode(enc_ptr, rc, lan->recv_buf, &recv_buf_len)) {
		goto decode_failure;
	}

	rc = al_aes_cbc_decrypt(lan->aes_rx, lan->recv_buf,
	...
	hmac_ctx = al_hmac_sha256_new();
	if (!hmac_ctx) {
		CLIENT_LOGF(LOG_ERR, "hmac alloc failure");
		goto remove_client;
	}
	al_hmac_sha256_init(hmac_ctx,
	    lan->app_sign_key, sizeof(lan->app_sign_key));
	al_hmac_sha256_update(hmac_ctx, lan->recv_buf, len);
	al_hmac_sha256_final(hmac_ctx, packet_sign);
	al_hmac_sha256_free(hmac_ctx);

	log_bytes(MOD_LOG_CLIENT, LOG_SEV_DEBUG2,
	    lan->recv_buf, len, "lan_dec_rx");

	if (memcmp(packet_sign, dec_sign, AL_HASH_SHA256_SIZE)) {
	...
	if (props) {
		...
		iter_rc = jsmn_array_iterate(&parser, props,
		...
		if (cmds) {
			...
			lan->recv_cmds = 1;
			iter_rc = jsmn_array_iterate(&parser, cmds,
					...
					struct client_lan_reg *lan)
{
	...
	lan->send_seq_no++;
	server_req_done_callback(cmd_req);
}
...
void client_lan_reset(struct client_state *state)
{
	...
	for (lan = client_lan_reg;
	    lan < &client_lan_reg[CLIENT_LAN_REGS]; lan++) {
		if (lan->uri[0] != '\0') {
			client_lan_state_update(lan, ALS_INACTIVE);
		}
		client_lan_reset_one(lan);
	}
	state->dest_mask &= ~NODES_LAN;
	state->valid_dest_mask &= ~NODES_LAN;
	state->echo_dest_mask &= ~NODES_LAN;
}
...
static void client_lan_remove(struct client_lan_reg *lan)
{
	...
	if (!lan->valid_key) {
		client_lan_state_update(lan, ALS_KEY_EXCH_FAILED);
	} else {
		client_lan_state_update(lan, ALS_INACTIVE);
	}
	client_lan_clearout(lan);
	client_prop_send_done(state, 0, NULL, lan->mask,
	    &lan->http_client);
	client_finish_echo(state, lan->mask);
...
static void client_err_lan_cb(struct http_client *hc)
{
	...
	ASSERT(client_locked);
	client_timer_cancel(&lan->timer);
	client_log(LOG_DEBUG "http err %d, lan #%u %s", hc->hc_error, lan->id,
	    client_conn_state_string(lan->conn_state));
	...
	if (hc->hc_error == HC_ERR_HTTP_STATUS &&
		...
		case CS_WAIT_ECHO:
			...
			break;
		case CS_WAIT_POST:
			client_prop_send_done(state, 1, NULL, lan->mask, hc);
			lan->send_seq_no++;
			break;
...
invalid:
	...
	ipaddr = ipaddr_fmt_str_to_ipv4(buf);
	if (!ipaddr) {
		goto invalid;
	}
	parse->host_addr = ipaddr;
	...
	CLIENT_LOGF(LOG_DEBUG2, "ip %s port %lu notify %u",
	    ipaddr_fmt_ipv4_to_str(ipaddr, buf, sizeof(buf)),
	    port, parse->notify);
	...
	if (adap_wifi_in_ap_mode() && len >= 0) {
		...
		if (ayla_base64_decode(p, len, tmp_str, &key_len)) {
			al_os_mem_free(key);
			goto invalid;
		}
		p = tmp_str;
		if (al_rsa_pub_key_set(key, tmp_str, key_len) <
		...
		int post)
{
	...
	for (reg = client_lan_reg;
		...
		if (best == NULL && !reg->valid_key) {
			best = reg;
		}

		if (reg->host_addr == parse->host_addr &&
		    reg->http_client.host_port == parse->port &&
		    !strncmp(reg->uri, parse->uri, sizeof(reg->uri))) {
			return reg;
		}
	}
	return post ? best : NULL;
}

/*
 * Determine which interface (if any) is local to the given IP address.
 */
static enum client_lan_if_type client_lan_get_if_type(u32 addr)
{
	...
	if (!addr || !~addr) {
		return LAN_IF_NON_LOCAL;	/* 0.0.0.0 or 255.255.255.255 */
	}
	for (type = AL_NET_IF_DEF; type < AL_NET_IF_MAX; type++) {
	...
	struct lan_parse *parse)
{
	...
	if_type = client_lan_get_if_type(parse->host_addr);
	parse->if_type = if_type;
#ifdef AYLA_WIFI_SUPPORT
	if (if_type == LAN_IF_AP) {
		if (!adap_wifi_in_ap_mode()) {
			CLIENT_LOGF(LOG_WARN, "AP mode down");
			return HTTP_STATUS_FORBID;
		}
		return 0;
	}
#endif /* AYLA_WIFI_SUPPORT */
	if (if_type != LAN_IF_LOCAL) {
		CLIENT_LOGF(LOG_WARN, "IP not local");
		return HTTP_STATUS_FORBID;
	}
	if (!(lcf->enable_mask & LAN_CONF_ENABLE_LAN_BIT) ||
	    cf->lan_disable) {
		return HTTP_STATUS_NOT_FOUND;
...
static void client_lan_timeout(struct timer *timer)
{
	...
	client_log(LOG_DEBUG "timeout, lan #%u %s",
	    lan->id, client_conn_state_string(lan->conn_state));
	http_client_abort(hc);
	client_lan_buf_free_int(lan);
	switch (lan->conn_state) {
	case CS_WAIT_GET:
	...
	struct client_lan_reg *lan, struct lan_parse *parse)
{
	...
	timer_handler_init(&lan->timer, client_lan_timeout);
	lan->id = (lan - client_lan_reg) + 1;	/* id 0 is for ADS */
	lan->mask = NODES_LAN_BASE << (lan->id - 1);
	...
	if (lan->rsa_ke) {
		 ...
		 */
		al_random_fill(state->lanip.lanip_random_key,
		    sizeof(state->lanip.lanip_random_key));
	}
	lan->mtime = clock_ms();
	...
	lan->host_addr = parse->host_addr;
	ipaddr_fmt_ipv4_to_str(lan->host_addr, hc->host, sizeof(hc->host));
	...
	client_lan_state_update(lan, ALS_ACTIVE);
...
static void client_json_lan_post(struct server_req *req)
{
	...
	/*
	 * "dsn" in query string is not required, but if used, must match.
	 */
	dsn = server_get_arg_by_name(req, "dsn", dsn_buf, sizeof(dsn_buf));
	if (dsn && strcmp(dsn, conf_sys_dev_id)) {
		server_log(LOG_WARN "local_reg POST DSN %s mismatch", dsn);
			/* TODO don't check in the debug log */
	...
	status = client_lan_check_precon(state, &parse);
	if (status) {
		goto put_bad_status;
	}
...
static void client_lan_refresh(struct client_lan_reg *lan, u8 notify)
{
	...
	client_log(LOG_DEBUG2 "refresh, lan #%u", lan->id);
	lan->refresh_count++;
...
static int client_lan_send_cmd(struct client_lan_reg *lan)
{
	...
	cmd_req->resp_len_limited = 1;
	cmd_req->resp_len_rem = lan->recv_buf_len;
