...
#define PROP_MAX_DPMETA		4	/* max num of dp metadata
					   key-value pairs */
#define PROP_ACK_ID_LEN		36	/**< Max len of an ack_id */
...
/**
 * Structure of property's ack.
 */
struct prop_ack {
	char id[PROP_ACK_ID_LEN + 1];	/**< ack id of property */
	u8 status;			/**< status of ack */
	s32 msg;			/**< ack message */
	u8 src;				/**< src of ack */
};
...
struct prop {
	const char *name;
	u8	fmt_flags;
	u8	echo:1;		/* echo by property manager, client, or LAN */
	u8	prop_mgr_echo:1; /* echo by the property manager */
	u8	send_dest;	/* destinations (internal field) */
	u8	is_dl;		/* 1 if file download */
	...
	struct prop_ack *ack;	/**< property acknowledgement */
};
...
struct prop_recvd {
	...
	u8 no_echo:1;   /* don't echo to cloud. per-prop version of auto_sync */
	u8 src;
	u8 meta_idx;		/* count of received metadata items */
	void *arg;		/* request-specific arg (indicates req ID) */
	struct prop_dp_meta metadata[PROP_MAX_DPMETA + 1];
};
...
enum prop_cb_status {
	...
	PROP_CB_INVAL_FMT,	/* value format improper (e.g., not JSON) */
	PROP_CB_UNK_PROP,	/* property not found by service */
	PROP_CB_TOO_MANY,	/* too many requests for service */
	PROP_CB_RETRY,		/* retry operation */
};
...
/*
 * Create a property datapoint from name, value, etc.
 */
