		    ...
		    const char *msg, void *arg), void *arg)
{
	...
	char *msg;
	char *bp;
	struct log_buf_ctxt *ctxt = NULL;

	/*
	 * Get log into scratch buf.
	 */
	buf = al_os_mem_alloc(LOG_SIZE);
	if (!buf) {
		return;
	}
	bp = buf;
	...
	if (!ctxt) {
		server_log(LOG_WARN "failed to open log buffer %u", snapshot);
		goto out;
	}

	log_buf_seq_set_tail(ctxt, lines);
	while (lines) {
		len = log_buf_get_next(ctxt, bp, buf_len);
		if (len <= 0) {
			goto out;
		}
	 ...
	 struct log_msg_head *head, const char *msg, void *arg)
{
	...
	server_put(req, "%s{\"mtime\":%lu,\"mod\":\"%s\",\"severity\":\"%s\","
	    ...
	    *sep, head->mtime, modname ? modname : "",
	    log_sev_get((enum log_sev)head->sev), msg);
	*sep = ",";
}
...
void log_page_mods_json_get(struct server_req *req)
{
	...
	for (mod = log_mods, mod_nr = 0; mod_nr < LOG_MOD_CT; mod_nr++, mod++) {
		...
		server_put(req, "%s{\"name\":\"%s\",\"levels\":",
		    sep, mod_name);
		sep = "[";
		for (sev = LOG_SEV_NONE; sev < LOG_SEV_LIMIT; sev++) {
	...
	jsmntok_t *obj, void *dryrun)
{
	...
	if (log_mask_change(name, (enum log_mask)change.value,
	    (enum log_mask)(change.mask & ~change.value))) {
