		...
		void *arg, struct ame_kvp *kvp)
{
	...
	size_t len = sizeof(prop->name);

	(void)dec->get_utf8(kvp, NULL, prop->name, &len);
	return AE_OK;
}
		...
		void *arg, struct ame_kvp *kvp)
{
	...
	if (kvp->type == AME_TYPE_UTF8) {
		memcpy(prop->val + prop->offset, buf, len);
	} else {
		memcpy(prop->val + prop->offset, (char *)kvp->value, len);
	}
...
const struct name_val prop_types[] = {
	...
	{ .name = "message", .val = ATLV_MSG_TYPE },
	{ .name = NULL, .val = ATLV_INVALID }
};

static int client_prop_type(const struct ame_decoder *dec,
		void *arg, struct ame_kvp *kvp)
{
	struct prop_recvd *prop = &prop_recvd;
	char buf[32];
	...
	if (!err) {
		prop->type = (enum ayla_tlv_type)
		    lookup_by_name(prop_types, buf);
	}
	return 0;
}

static int client_dpmeta_item(const struct ame_decoder *dec, unsigned int key,
		struct ame_kvp *kvp)
{
	struct prop_recvd *prop = &prop_recvd;
	...
	if (prop->meta_idx >= PROP_MAX_DPMETA) {
		return 0;
	}
	meta = &prop->metadata[prop->meta_idx];
	if (key) {
		len = sizeof(meta->key);
		err = dec->get_utf8(kvp, NULL, meta->key, &len);
	} else {
		len = sizeof(meta->value);
		err = dec->get_utf8(kvp, NULL, meta->value, &len);
	}
	if (err) {
		meta->key[0] = '\0';
		meta->value[0] = '\0';
	...
	if (meta->key[0] && meta->value[0]) {
		prop->meta_idx++;
	}
	return 0;
}

static int client_dpmeta_key(const struct ame_decoder *dec,
		void *arg, struct ame_kvp *kvp)
{
	return client_dpmeta_item(dec, 1, kvp);
}

static int client_dpmeta_val(const struct ame_decoder *dec,
		void *arg, struct ame_kvp *kvp)
{
	return client_dpmeta_item(dec, 0, kvp);
}

static int client_prop_accept_dpmeta(const struct ame_decoder *dec,
		void *arg, struct ame_kvp *kvp)
{
	struct prop_recvd *prop = &prop_recvd;
...
static int client_prop_ack_id(const struct ame_decoder *dec,
		void *arg, struct ame_kvp *kvp)
{
	struct prop_recvd *prop = &prop_recvd;
	size_t len = sizeof(prop->ack_id);

	(void)dec->get_utf8(kvp, NULL, prop->ack_id, &len);
	return 0;
}
...
struct prop *client_prop_get_to_send(u8 dest_mask)
{
	...
	if (state->echo_dest_mask & dest_mask) {
		ASSERT(echo_prop);
	...
	if (!state->get_echo_inprog && (state->dest_mask & dest_mask)) {
		prop = prop_mgr_get_send_prop();
		ASSERT(prop);
		return prop;
	}
...
void client_prop_finish_send(u8 success, u8 dest_mask)
{
	...
	if (state->get_echo_inprog) {
		client_finish_echo(&client_state, dest_mask);
	} else {
		client_prop_send_done_ext(success, NULL, dest_mask);
	}
	client_wakeup();
}

static enum ada_err client_echo_prop_from_service(struct client_state *state)
{
	struct prop_recvd *prop = &prop_recvd;
	...
	if (prop->type == ATLV_LOC || prop->type == ATLV_MSG_BIN ||
	    prop->type == ATLV_MSG_UTF8 || prop->type == ATLV_MSG_JSON) {
		goto clear_prop;
	}
#endif
...
static int client_accept_prop(const struct ame_decoder *dec,
		void *arg, struct ame_kvp *kvp)
{
	struct client_state *state = &client_state;
	struct prop_recvd *prop = &prop_recvd;
...
static const struct ame_tag client_ame_dpmeta_inner[] = {
	AME_TAG("name", NULL, client_dpmeta_key),
	AME_TAG("value", NULL, client_dpmeta_val),
	AME_TAG(NULL, NULL, NULL)
};

static const struct ame_tag client_ame_dpmeta[] = {
	AME_TAG("", client_ame_dpmeta_inner, NULL), /* outer unnamed object */
	AME_TAG(NULL, NULL, NULL)
};

static const struct ame_tag client_ame_prop_tags[] = {
	AME_TAG("name", NULL, client_prop_name),
	AME_TAG("value", NULL, client_prop_val),
	AME_TAG("base_type", NULL, client_prop_type),
	AME_TAG("id", NULL, client_prop_ack_id),
	AME_TAG("dp_meta", client_ame_dpmeta, client_prop_accept_dpmeta),
	AME_TAG(NULL, NULL, NULL)
};

static const struct ame_tag client_ame_prop_inner[] = {
	AME_TAG("property", client_ame_prop_tags, NULL),
	AME_TAG("schedule", client_ame_prop_tags, NULL),
	AME_TAG(NULL, NULL, NULL)
};

const struct ame_tag client_ame_prop[] = {	/* outer unnamed object */
	AME_TAG("", client_ame_prop_inner, client_accept_prop),
	AME_TAG(NULL, NULL, NULL)
};
				...
				void *arg, u8 dest, struct http_client *hc)
{
	...
	if (state->unexp_op) {
		 ...
		 */
		if (hc && hc->hc_error == HC_ERR_HTTP_STATUS &&
		    (dest & NODES_ADS) &&
		    hc->http_status >= 400 && hc->http_status < 500) {
	...
	if (rc != AE_OK) {
		...
		METRIC_INCR(M_PROP_SEND_STATUS, 0, status);
		METRIC_OP_END(M_PROP_SEND_LATENCY, 0);
		if (status == PROP_CB_DONE) {
			METRIC_INCR(M_PROP_SEND_SUCCESSES, 0, 0);
		} else {
			METRIC_INCR(M_PROP_SEND_FAILURES, 0, 0);
		}
#endif
	}
...
enum ada_err client_prop_send_done_ext(u8 success, void *arg, u8 dest)
{
	...
	err = client_prop_send_done(state, success, arg, dest, NULL);
	client_wakeup();
	return err;
}
...
enum ada_err client_get_prop_val(const char *name)
{
	...
	state->conn_state = CS_WAIT_PROP_GET;
...
enum ada_err client_continue_recv(void *arg)
{
	...
	client_log(LOG_DEBUG "%s: name=\"%s\" state %s", __func__,
	    prop->name, client_conn_state_string(state->conn_state));

	switch (state->request) {
	case CS_GET_VAL:
	case CS_GET_ALL_VALS:
	case CS_GET_CMDS:
	case CS_GET_COMMANDS:
	...
	if (state->cont_recv_hc) {
		if (state->cont_recv_hc->is_mqtt) {
			http2mqtt_client_continue_recv(state->cont_recv_hc);
		} else {
...
enum ada_err client_prop_set(struct prop_recvd *prop)
{
	...
	err = ada_prop_mgr_meta_set(name, type, prop->val, val_len,
	    &prop->offset, prop->src, prop->arg, prop->ack_id,
	    prop->prop.dp_meta);
	/*
	 * AE_ABRT is an indication to abort subsequent processing (echos)
	 * of this property.
	 */
	if (err && err != AE_ABRT && err != AE_BUF) {
