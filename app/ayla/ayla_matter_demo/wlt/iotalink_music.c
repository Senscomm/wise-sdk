...
#define KEY_SCAN_INTERVAL_MS   20      // 按键扫描间隔(ms)
#define LONG_PRESS_TIME_MS     3000     // 长按触发阈值(ms)，长按3000ms生效
...
// 按键状态机（新增：用于管理长按/短按状态）
typedef enum {
    KEY_STATE_IDLE = 0,    // 空闲状态：无按键
    KEY_STATE_PRESS,       // 按下状态：短按检测中
    KEY_STATE_LONG         // 长按状态：已触发长按动作
} key_state_t;
...
static void adc_key_long_action(adc_key_t key)
{
	...
	{
        case KEY1:
            my_printf("KEY1 长按：触发长按功能\n");
            break;
            
        case KEY2:
            my_printf("KEY2 长按：触发长按功能\n");

            break;
            
        case KEY3:
            my_printf("KEY3 长按三秒进入复位配网模式 \n");
    ...
    while (1)
	{
        ...
        switch(key_state)
        {
            ...
            case KEY_STATE_IDLE:
                if(cur_key != KEY_NONE)
                {
                    wlt_ms_delay(KEY_DEBOUNCE_MS); // 消抖
                    cur_key = adc_key_scan();
                    if(cur_key != KEY_NONE)
                    {
                        valid_key = cur_key;        // 保存确认后的按键值
