 ...
 * This is required by the SDK to be in RAM.
 */
static wifi_country_t demo_wifi_country = {
	.cc = DEMO_WIFI_CC,
...
static void demo_wifi_set_country_code(void)
{
	...
	rc = wise_wifi_set_country(&demo_wifi_country);
	if (rc != WISE_OK) {
...
void demo_wifi_init(void)
{
	...
	snprintf(hostname, sizeof(hostname),
	    DEMO_HOSTNAME_PREFIX "%2.2x%2.2x",
	    cf->mac_addr[4], cf->mac_addr[5]);
	log_put(LOG_INFO "%s: hostname %s", __func__, hostname); /* TODO */
