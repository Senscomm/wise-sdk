...
static int IRAM_OPT bl0937_cf_isr_handler(uint32_t pin, void* arg)
{
...
static int IRAM_OPT bl0937_cf1_isr_handler(uint32_t pin, void* arg)
{
    bl0937_dev_t *dev = (bl0937_dev_t *)arg;
    uint64_t now = time_now();
    ...
    if ((now - dev->first_cf1_interrupt) > dev->pulse_timeout_us) {
        ...
        dev->mode = 1 - dev->mode;
        scm_gpio_write(dev->setpin_io, dev->mode);
...
esp_err_t bl0937_create(bl0937_handle_t *handle, driver_bl0937_config_t *config)
{
    ...
    if (dev == NULL) {
        WISE_LOGE(TAG, "Failed to allocate memory for BL0937 device");
    ...
    scm_gpio_disable_interrupt(config->cf_gpio);
    scm_gpio_disable_interrupt(config->cf1_gpio);
    ...
    scm_gpio_enable_interrupt(config->cf_gpio, SCM_GPIO_INT_RISING_EDGE, bl0937_cf_isr_handler, dev);
    scm_gpio_enable_interrupt(config->cf1_gpio, SCM_GPIO_INT_RISING_EDGE, bl0937_cf1_isr_handler, dev);

    calculate_default_multipliers(dev);
    dev->mode = dev->current_mode;

    scm_gpio_write(config->sel_gpio, dev->mode);
...
void IRAM_OPT bl0937_setmode(bl0937_handle_t handle, bl0937_mode_t mode_type)
{
    ...
    scm_gpio_write(dev->setpin_io, gpio_level);
    dev->mode = gpio_level;

    dev->last_cf1_interrupt = dev->first_cf1_interrupt = time_now();
}
...
{
    bl0937_dev_t *dev = (bl0937_dev_t *)handle;
    if ((time_now() - dev->last_cf_interrupt) > dev->pulse_timeout_us) {
        dev->sensor_power_pulse_width = 0;
    }
}
...
{
    bl0937_dev_t *dev = (bl0937_dev_t *)handle;
    if ((time_now() - dev->last_cf1_interrupt) > dev->pulse_timeout_us) {
        if (dev->mode == dev->current_mode) {
            dev->sensor_current_pulse_width = 0;
        } else {
            dev->sensor_voltage_pulse_width = 0;
...
void IRAM_OPT bl0937_reset_multipliers(bl0937_handle_t handle)
{
    bl0937_dev_t *dev = (bl0937_dev_t *)handle;
    calculate_default_multipliers(dev);
    WISE_LOGI("BL0937", "Multipliers reset to defaults: V=%.0f, I=%.0f, P=%.0f",
