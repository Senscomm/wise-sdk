...
#else
            ESP_LOGE(TAG, "Failed to set client configurations, [0x%04X]", esp_ret);
#endif
            goto exit;
        }
...
#else
            ESP_LOGE(TAG, "Failed to set server configurations, [0x%04X]", esp_ret);
#endif
            goto exit;
        }
    }
...
#else
        ESP_LOGE(TAG, "create_ssl_handle failed, [0x%04X]", esp_ret);
#endif
