...
esp_err_t esp_create_mbedtls_handle(const char *hostname, size_t hostlen, const void *cfg, esp_tls_t *tls, void *server_params)
{
    ...
    if (tls->role == ESP_TLS_CLIENT) {
...
#else
            ESP_LOGE(TAG, "Failed to set client configurations, returned [0x%04X]", esp_ret);
#endif
            goto exit;
        }
        ...
        if (tls_ver == ESP_TLS_VER_TLS_1_3) {
            ...
            mbedtls_ssl_conf_max_tls_version(&tls->conf, MBEDTLS_SSL_VERSION_TLS1_2);
#else
            ESP_LOGD(TAG, "Setting TLS version to 0x%4x.%4x", MBEDTLS_SSL_MAJOR_VERSION_3,
...
#else
            ESP_LOGE(TAG, "Failed to set server configurations, returned [0x%04X]", esp_ret);
#endif
            goto exit;
        }
    }
...
#ifndef __WISE__ /* Not supported and not referred yet. */
...
#else
            ESP_LOGE(TAG, "Failed to set client pki context for the DS peripheral, returned [0x%04X]", ret);
#endif
            return esp_ret;
        }
#else
...
#else
        ESP_LOGE(TAG, "create_ssl_handle failed, returned [0x%04X]", esp_ret);
#endif
