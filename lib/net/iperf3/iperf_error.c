...
iperf_strerror(int int_errno)
{
	...
	case IESETBUF2:
	    ...
	    break;
	case IETCPBLKSIZE:
	    snprintf(errstr, len, "Block size too large. use -l option. (maximum = %d bytes)", DEFAULT_TCP_BLKSIZE);
	    break;
	case IEUDPBLKSIZE:
	    snprintf(errstr, len, "Block size too large. use -l option. (maximum = %d bytes)", MAX_UDP_BLKSIZE);
	    break;
    }
