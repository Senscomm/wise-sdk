...
usage_long(void)
{
    printf(usage_longstr, DEFAULT_NO_MSG_RCVD_TIMEOUT, UDP_RATE / (1024*1024), DURATION, DEFAULT_TCP_BLKSIZE / 1024, DEFAULT_UDP_BLKSIZE);
}
...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    optind = 0;
    while ((flag = getopt_long(argc, argv, "p:f:i:D1VqJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
        switch (flag) {
            case 'p':
		...
		if (test->settings->unit_format == 'k' ||
            ...
            case 'i':
                ...
                if ((test->stats_interval < MIN_INTERVAL || test->stats_interval > MAX_INTERVAL) && test->stats_interval != 0) {
                    i_errno = IEINTERVAL;
                    return -1;
                } else if (test->stats_interval == 0) {
                    /* if `-i` arg is 0, we need stats interval to be non-zero */
                    test->stats_interval = DEFAULT_INTERVAL;
                }
                break;
            case 'D':
		test->daemon = 1;
		...
		client_flag = 1;
            ...
            case OPT_RCV_TIMEOUT:
                ...
                test->settings->rcv_timeout.tv_sec = rcv_timeout_in / SEC_TO_mS;
                test->settings->rcv_timeout.tv_usec = (rcv_timeout_in % SEC_TO_mS) * mS_TO_US;
                rcv_timeout_flag = 1;
	        break;
            case 'A':
#if defined(HAVE_CPU_AFFINITY)
    ...
    if (test->role == 'c' && (server_rsa_private_key || test->server_authorized_users)){
        i_errno = IESERVERONLY;
        return -1;
    } else if (test->role == 'c' && rcv_timeout_flag && test->sender){
...
iperf_init_test(struct iperf_test *test)
{
    ...
    /* Init each stream. */
    if (iperf_time_now(test, &now, NULL) < 0) {
	i_errno = IEINITTEST;
	return -1;
    }
    SLIST_FOREACH(sp, &test->streams, streams) {
...
iperf_create_send_timers(struct iperf_test * test)
{
	...
	if (test->settings->rate != 0) {
	    cd.p = sp;
	    sp->send_timer = tmr_create(test, (struct timeval*) 0, send_timer_proc, cd, test->settings->pacing_timer, 1);
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    testp->settings->rcv_timeout.tv_sec = DEFAULT_NO_MSG_RCVD_TIMEOUT / SEC_TO_mS;
    testp->settings->rcv_timeout.tv_usec = (DEFAULT_NO_MSG_RCVD_TIMEOUT % SEC_TO_mS) * mS_TO_US;
...
iperf_stats_callback(struct iperf_test *test)
{
	...
	irp = TAILQ_LAST(&rp->interval_results, irlisthead);
        ...
        iperf_time_now(test, &rp->end_time, NULL);
	...
	/* Can't find any reason to keep all interval results */
	{
...
iperf_new_stream(struct iperf_test *test, int s)
{
    ...
    /*
     * UDP: Due to reassemble fail, length limit as MAX_UDP_BLKSIZE (refer IP_REASS_MAX_PBUFS)
     * TCP: Due to lack of heap size, we change DEFAULT_TCP_BLKSIZE 128*1024 to 8*1024
     * */
    if (test->protocol->id == Pudp && test->settings->blksize > MAX_UDP_BLKSIZE) {
    	free(sp);
    	i_errno = IEUDPBLKSIZE;
    	return NULL;
    } else if (test->protocol->id == Ptcp && test->settings->blksize > DEFAULT_TCP_BLKSIZE) {
    	free(sp);
    	i_errno = IETCPBLKSIZE;
    	return NULL;
    }
