...
create_client_timers(struct iperf_test * test)
{
    ...
    if (test->stats_interval != 0) {
        test->stats_timer = tmr_create(test, &now, client_stats_timer_proc, cd, test->stats_interval * SEC_TO_US, 1);
    ...
    if (test->reporter_interval != 0) {
        test->reporter_timer = tmr_create(test, &now, client_reporter_timer_proc, cd, test->reporter_interval * SEC_TO_US, 1);
...
iperf_run_client(struct iperf_test * test)
{
	...
	if (timeout) {
		...
		*  be 0 which means not block at all, or a value larger than 1000us, other wise,
