...
create_server_timers(struct iperf_test * test)
{
    ...
    if (test->duration != 0 ) {
        test->done = 0;
        test->timer = tmr_create(test, &now, server_timer_proc, cd, (test->duration + test->omit + 5) * SEC_TO_US, 0);
        if (test->timer == NULL) {
            i_errno = IEINITTEST;
            return -1;
        }
    ...
    if (test->stats_interval != 0) {
        test->stats_timer = tmr_create(test, &now, server_stats_timer_proc, cd, test->stats_interval * SEC_TO_US, 1);
    ...
    if (test->reporter_interval != 0) {
        test->reporter_timer = tmr_create(test, &now, server_reporter_timer_proc, cd, test->reporter_interval * SEC_TO_US, 1);
...
iperf_run_server(struct iperf_test *test)
{
    ...
    struct timeval poll;
    struct timeval *timeout = NULL, _timeout = {0};
    int c = -1;
    int64_t t_usecs;
    ...
    (void) iperf_time_now(test, &last_receive_time, NULL); /* Initialize last time something was received */

    test->state = IPERF_START;
    streams_accepted = 0;
    rcv_timeout_us = (test->settings->rcv_timeout.tv_sec * SEC_TO_US) +
        test->settings->rcv_timeout.tv_usec;
    ...
    while (test->state != IPERF_DONE) {
        ...
        (void) iperf_time_now(test, &now, NULL);
        tmr_timeout(test, &now, &timeout);
        ...
        if (timeout) {
            ...
            /* in the implementation of function 'select', the absolute timeout value should
            *  be 0 which means not block at all, or a value larger than 1000us, otherwise,
            *  the calculation of blocking ticks would be
            *  portMAX_DELAY, which will cause blocking forever
            */
	    ...
	    cleanup_server(test);
            ...
            if (iperf_time_diff(&now, &last_receive_time, &diff_time) == 0) {
                ...
                if (!test->sender && t_usecs > rcv_timeout_us) {
	...
	if (result > 0) {
            (void) iperf_time_now(test, &last_receive_time, NULL);
