...
*/
extern void tmr_timeout( struct iperf_test *test, struct timeval* nowP, struct timeval **timeout) /* __attribute__((hot)) */;

/* Run the list of timers. Your main program needs to call this every so often,
** or as indicated by tmr_timeout().
*/
extern void tmr_run( struct iperf_test *test, struct timeval* nowP ) /* __attribute__((hot)) */;

/* Reset the clock on a timer, to current time plus the original timeout. */
extern void tmr_reset( struct iperf_test *test, struct timeval* nowP, Timer* timer );
...
*/
extern void tmr_cancel( struct iperf_test *test, Timer* timer );

/* Clean up the timers package, freeing any unused storage. */
extern void tmr_cleanup( struct iperf_test *test );

/* Cancel all timers and free storage, usually in preparation for exiting. */
extern void tmr_destroy( struct iperf_test *test );

extern int iperf_time_now(struct iperf_test *test, struct timeval *time1, struct timezone *tz);
extern void iperf_time_init(struct iperf_test *test);

extern int iperf_time_diff(const struct timeval *time1, const struct timeval *time2, struct timeval *diff);
extern uint64_t iperf_time_in_usecs(struct timeval *time);
#endif /* __TIMER_H */
