...
static uint32_t _get_random(void)
{
	struct device *dev;
	uint32_t rand_val;

	dev = device_get_by_name("trng");
	if (!dev) {
		assert(0);
	}

	crypto_trng_get_rand(dev, (uint8_t *)&rand_val, 4);
...
#else
            client->auth_data->cnonce = ((uint64_t)_get_random() << 32) + _get_random();
#endif
...
#else
                ESP_LOGE(TAG, "transport_read: error - %d", err);
#endif
            }
