...
static int md5_printf(char *md, const char *fmt, ...)
{
    ...
    if (mbedtls_md5_update_ret(&md5_ctx, buf, len) != 0) {
		goto exit;
	}
...
static int sha256_sprintf(char *sha, const char *fmt, ...)
{
    ...
    mbedtls_sha256_update(&sha256, buf, len);
...
#else
        rc = asprintf(&temp_auth_str, ", qop=%s, nc=%08x, cnonce=\"%016""llx""\"", auth_data->qop, auth_data->nc, auth_data->cnonce);
#endif
        if (rc < 0) {
            ESP_LOGE(TAG, "asprintf() returned: %d", rc);
            ret = ESP_FAIL;
