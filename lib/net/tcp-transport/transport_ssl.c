...
#ifdef __WISE__
/* XXX: why do we have to have separate callbacks for ssl and tcp here when we
 *      already have encapsulated all the differences in esp-tls layer?
 *      For example, if we just use the same esp_tls_conn_new_sync for both ssl and tcp,
 *      we will be able to use the same callbacks for write and read.
 *      Is this a tradeoff situation between memory footprint and speed?
 */
#endif
