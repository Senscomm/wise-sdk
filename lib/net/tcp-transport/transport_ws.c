...
{
	uint32_t ret;
	struct device *dev = device_get_by_name("trng");

	if (dev == NULL) {
		return -1;
	}

	ret = crypto_trng_get_rand(dev, buf, buflen);
	if (ret != 0) {
		return -1;
	}
...
#else
    const char *user_agent_ptr = (ws->user_agent) ? (ws->user_agent) : "wise-websocket-client";
#endif
#ifdef CONFIG_WS_DYNAMIC_BUFFER
    if (!ws->buffer) {
