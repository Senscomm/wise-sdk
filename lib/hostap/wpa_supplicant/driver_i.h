...
static inline int wpa_drv_set_ssid(struct wpa_supplicant *wpa_s, const u8 *ssid, int ssid_len)
{
	if (wpa_s->driver->set_ssid) {
		return wpa_s->driver->set_ssid(wpa_s->drv_priv, ssid, ssid_len);
	}
	return -1;
}
...
static inline int wpa_drv_set_twt(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->set_twt) {
		return wpa_s->driver->set_twt(wpa_s->drv_priv, &wpa_s->twt_params);
	}
	return -1;
}
#endif
#if defined(__WISE__) && defined(poll)
/*
 * poll is defined to be lwip_poll by lwip, which causes
 * a build error for the function wpa_drv_poll(). As a workaround
 * we enclose the function with this silly #define/#undef.
 */
#define _poll poll
#undef poll
#endif
...
#if defined(__WISE__) && defined(_poll)
#define poll _poll
#undef _poll
#endif
...
static inline void
wpa_drv_set_scan_entry(struct wpa_supplicant *wpa_s,
		struct wpa_scan_results *scan_res,
		void *ieee80211_sr)
{
	if (!(scan_res && ieee80211_sr))
		return;
	wpa_add_scan_entry(scan_res, ieee80211_sr);
}
