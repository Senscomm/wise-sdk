...
struct reconnect_policy {
	...
	int pending_flag;
	int evt_ignored;
	system_event_t saved_evt;
	bool disable_wpas_retry;	/* this is used to disable the retry by wpa_supplicant itself */
	bool resume_scan; /* resume the reconnect by trigger the scan */
	bool reconnect_op; /* wise reconnect operation so it will take over reconnection policy during this period*/
};
...
#define WILDCARD_SSID_SCAN ((struct wpa_ssid *) 1)
...
#ifdef __WISE__
	/* to protect bss, bss_id, last_scan_res, etc. */
	osMutexId_t bss_lock;
#endif
...
#if !defined(__WISE__) || defined(PCSC_FUNCS) || defined(CONFIG_EAP_PROXY)
	char imsi[20];
	int mnc_len;
#endif
	...
	unsigned int manual_scan_show_hidden:1;
	unsigned int manual_scan_spec_ssid:1;
	unsigned int manual_scan_dwell_min;
	unsigned int manual_scan_dwell_max;
...
#define MAX_SCAN_ID 1
	...
	struct {
		...
		int freq;
#ifdef CONFIG_FILS
#define ASSOC_REQ_BUF_SIZE 1500
#elif CONFIG_P2P
#define ASSOC_REQ_BUF_SIZE 300
#else
#define ASSOC_REQ_BUF_SIZE 80
#endif
		u8 assoc_req_ie[ASSOC_REQ_BUF_SIZE];
...
#if !defined(__WISE__) || defined(CONFIG_P2P) || defined(CONFIG_DPP)
	unsigned int off_channel_freq;
#endif
#if !defined(__WISE__) || defined(CONFIG_OFFCHANNEL)
...
#endif
	struct reconnect_policy reconnect_params;
#ifdef __WISE__
	int fast_conct_freq;
	u8 disassoc_evt;
#endif
...
void wpa_supplicant_stop_reconnect_timer(struct wpa_supplicant *wpa_s);
