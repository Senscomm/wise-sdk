...
#if defined(__WISE__) && defined(CONFIG_IEEE80211AX)
...
struct he_ppe_threshold {
 ...
 /***********************************************************
  *  Warning do not change the parameter sequence in wpa_twt_params
  ***********************************************************/
...
#if CONFIG_ELIMINATE_WEAK_RSSI_SCAN_ENTRY
	u8 desire;
#endif
	size_t ie_len;
...
struct wpa_driver_scan_params {
	...
	u64 scan_cookie;
	 ...
	 /**
	  * duration_min - Min. dwell time on each channel
	  *
	  * This optional parameter can be used to set the minimum dwell time
	  * on each channel. In TUs.
	  */
	 u16 duration_min;
...
#ifdef __WISE__
	/**
	 * passive - Whether this acan is passive or not
	 *
	 * If this is set, station will not send probe request on each channel,
	 * but instead wait for beacon frames for duration.
	 */
	s8 passive;
...
struct wpa_driver_ops {
	...
	/**
	 * set_ssid - Set SSID to be connected
	 * @priv: Private driver interface data
	 * @buf: SSID
	 ...
	 */
	int (*set_ssid)(void *priv, const u8 *ssid, int ssid_len);

	/**
	 * set_bssid - Set BSSID to be connected
	 * @priv: Private driver interface data
	 * @buf: BSSID
	 * Returns: 0 on success, -1 on failure
	 *
...
#endif /* CONFIG_SUPPORT_MINIMUM_SCAN_TABLE */
...
#ifdef CONFIG_SUPPORT_TWT
	int (*set_twt)(void *priv, const struct wpa_twt_params *params);
#endif
...
enum wpa_event_type {
	...
	/**
	 * EVENT_SCAN_ENTRY - Scan results indicate
	 *
	 * This event can be used to report a scan entry to supplicant
	 */
	EVENT_SCAN_ENTRY,

#endif /* CONFIG_SUPPORT_SCAN_ENTRY_NOTIFY */
	/**
...
#if CONFIG_SUPPORT_SCAN_ENTRY_NOTIFY
	/**
	 * struct scan_entry - Data for EVENT_SCAN_ENTRY events
	 * @ ieee80211_sr: net80211 report a scan result
	 */
	struct scan_entry {
		struct ieee80211req_scan_result *ieee80211_sr;
	} scan_entry;
...
void wpa_add_scan_entry(struct wpa_scan_results *res,
			      void *sr);
#endif
