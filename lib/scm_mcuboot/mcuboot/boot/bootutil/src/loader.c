...
struct swap_progress {
    uint32_t total_sz;
    uint32_t img_oft;
};

static struct swap_progress prog;

void
boot_set_copy_size(uint32_t copy_sz)
{
    prog.total_sz = copy_sz;
    prog.img_oft = 0;
}

void
boot_update_progress(uint32_t wr_sz)
{
    uint32_t progress;

    if (prog.img_oft == 0) {
        if (prog.total_sz % wr_sz) {
            prog.total_sz = ((prog.total_sz / wr_sz) + 1) * wr_sz;
        }
    }

    prog.img_oft += wr_sz;

    progress = (prog.img_oft * 100) / prog.total_sz;

    printf("Swap Progress: %-8d of %d bytes [%d %%]\n",
            prog.img_oft, prog.total_sz, progress);
}
...
#ifdef CONFIG_SCM_MCUBOOT
    TARGET_STATIC uint8_t buf[4096] __attribute__((aligned(4)));
#else
...
#ifdef CONFIG_SCM_MCUBOOT
        if (fap_src->fa_id == FLASH_AREA_IMAGE_SCRATCH && fap_dst->fa_id == PRIMARY_ID && off_src == 0) {
            boot_update_progress(chunk_sz);
        }
#endif
