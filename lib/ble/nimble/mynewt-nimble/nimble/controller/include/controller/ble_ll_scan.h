...
__romfunc__ extern int (*ble_ll_scan_hci_set_params)(const uint8_t *cmdbuf, uint8_t len);

/* Turn scanning on/off */
__romfunc__ extern int (*ble_ll_scan_hci_set_enable)(const uint8_t *cmdbuf, uint8_t len);
__romfunc__ extern int (*ble_ll_scan_hci_set_ext_enable)(const uint8_t *cmdbuf, uint8_t len);

#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_EXT_ADV)
__romfunc__ extern int (*ble_ll_scan_hci_set_ext_params)(const uint8_t *cmdbuf, uint8_t len);
#endif

/*--- Controller Internal API ---*/
/* Initialize the scanner */
__romfunc__ extern void (*ble_ll_scan_init)(void);

#if (SCM2010)
/* Deinitialize the scanner */
extern __func_tab__ void (*ble_ll_scan_deinit)(void);
#endif

/* Reset the scanner */
__romfunc__ extern void (*ble_ll_scan_reset)(void);

/* Called when Link Layer starts to receive a PDU and is in scanning state */
__romfunc__ extern int (*ble_ll_scan_rx_isr_start)(uint8_t pdu_type, uint16_t *rxflags);

/* Called when Link Layer has finished receiving a PDU while scanning */
__romfunc__ extern int (*ble_ll_scan_rx_isr_end)(struct os_mbuf *rxpdu, uint8_t crcok);

/* Process a scan response PDU */
__romfunc__ extern void (*ble_ll_scan_rx_pkt_in)(uint8_t pdu_type, struct os_mbuf *om,
...
__romfunc__ extern void (*ble_ll_scan_sm_stop)(int chk_disable);

/* Resume scanning */
#if MYNEWT_VAL(BLE_LL_ROLE_OBSERVER)
__romfunc__ extern void (*ble_ll_scan_chk_resume)(void);
#else
...
__romfunc__ extern void (*ble_ll_scan_halt)(void);
