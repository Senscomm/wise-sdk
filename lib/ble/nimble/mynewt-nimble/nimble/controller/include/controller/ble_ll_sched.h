...
#if (SCM2010)
#define BLE_LL_SCHED_TICKS_PER_SLOT   (1250 * 20)
#else
 ...
 */
#if (SCM2010)
extern uint32_t g_ble_ll_sched_offset_ticks;
#else
...
__romfunc__ extern int (*ble_ll_sched_init)(void);

/* Remove item(s) from schedule */
__romfunc__ extern int (*ble_ll_sched_rmv_elem)(struct ble_ll_sched_item *sch);

__romfunc__ extern void (*ble_ll_sched_rmv_elem_type)(uint8_t type, sched_remove_cb_func remove_cb);

/* Schedule a new master connection */
struct ble_ll_conn_sm;
__romfunc__ extern int (*ble_ll_sched_conn_central_new)(struct ble_ll_conn_sm *connsm,
                                  struct ble_mbuf_hdr *ble_hdr, uint8_t pyld_len);

/* Schedule a new slave connection */
__romfunc__ extern int (*ble_ll_sched_conn_periph_new)(struct ble_ll_conn_sm *connsm);
...
__romfunc__ extern int (*ble_ll_sched_adv_new)(struct ble_ll_sched_item *sch,
                         ble_ll_sched_adv_new_cb cb, void *arg);

/* Schedule periodic advertising event */
__romfunc__ extern int (*ble_ll_sched_periodic_adv)(struct ble_ll_sched_item *sch, bool first_event);

__romfunc__ extern int (*ble_ll_sched_sync_reschedule)(struct ble_ll_sched_item *sch,
                                 uint32_t anchor_point,
                                 uint8_t anchor_point_usecs,
                                 uint32_t window_widening, int8_t phy_mode);
__romfunc__ extern int (*ble_ll_sched_sync)(struct ble_ll_sched_item *sch,
                      uint32_t beg_cputime, uint32_t rem_usecs, uint32_t offset,
                      int8_t phy_mode);

/* Reschedule an advertising event */
__romfunc__ extern int (*ble_ll_sched_adv_reschedule)(struct ble_ll_sched_item *sch,
                                uint32_t max_delay_ticks);

/* Reschedule and advertising pdu */
__romfunc__ extern int (*ble_ll_sched_adv_resched_pdu)(struct ble_ll_sched_item *sch);

/* Reschedule a connection that had previously been scheduled or that is over */
__romfunc__ extern int (*ble_ll_sched_conn_reschedule)(struct ble_ll_conn_sm * connsm);
 ...
 */
__romfunc__ extern int (*ble_ll_sched_next_time)(uint32_t *next_event_time);
...
__romfunc__ extern int (*ble_ll_sched_scan_aux)(struct ble_ll_sched_item *sch, uint32_t pdu_time,
                          uint8_t pdu_time_rem, uint32_t offset_us);
#endif

/* Stop the scheduler */
__romfunc__ extern void (*ble_ll_sched_stop)(void);

#if MYNEWT_VAL(BLE_LL_DTM)
__romfunc__ extern int (*ble_ll_sched_dtm)(struct ble_ll_sched_item *sch);
#endif
