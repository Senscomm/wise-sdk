...
#if (SCM2010)
/*
 * Telink needs more information besides the BLE PDU in the buffer
 */
#define BLE_PHY_MAX_PDU_LEN         (280)
#else
...
/* Initialize the PHY */
extern __func_tab__ int (*ble_phy_init)(void);

/* Set the PHY channel */
extern __func_tab__ int (*ble_phy_setchan)(uint8_t chan, uint32_t access_addr, uint32_t crcinit);

/* Set transmit start time */
extern __func_tab__ int (*ble_phy_tx_set_start_time)(uint32_t cputime, uint8_t rem_usecs);

/* Set receive start time */
extern __func_tab__ int (*ble_phy_rx_set_start_time)(uint32_t cputime, uint8_t rem_usecs);

/* Set the transmit end callback and argument */
extern __func_tab__ void (*ble_phy_set_txend_cb)(ble_phy_tx_end_func txend_cb, void *arg);

typedef uint8_t (*ble_phy_tx_pducb_t)(uint8_t *dptr, void *pducb_arg,
                                      uint8_t *hdr_byte);

/* Place the PHY into transmit mode */
extern __func_tab__ int (*ble_phy_tx)(ble_phy_tx_pducb_t pducb, void *pducb_arg, uint8_t end_trans);

/* Place the PHY into receive mode */
extern __func_tab__ int (*ble_phy_rx)(void);

/* Copies the received PHY buffer into the allocated pdu */
extern __func_tab__ void (*ble_phy_rxpdu_copy)(uint8_t *dptr, struct os_mbuf *rxpdu);

/* Set the transmit power */
extern __func_tab__ int (*ble_phy_txpwr_set)(int dbm);

/* Get highest allowed power from range */
extern __func_tab__ int (*ble_phy_txpower_round)(int dbm);

/* Get the transmit power */
extern __func_tab__ int (*ble_phy_txpwr_get)(void);

/* Set RX path power compensation value rounded to integer dB */
extern __func_tab__ void (*ble_phy_set_rx_pwr_compensation)(int8_t compensation);

/* Disable the PHY */
extern __func_tab__ void (*ble_phy_disable)(void);

#define BLE_PHY_WFR_ENABLE_RX       (0)
#define BLE_PHY_WFR_ENABLE_TXRX     (1)

extern __func_tab__ void (*ble_phy_wfr_enable)(int txrx, uint8_t tx_phy_mode, uint32_t wfr_usecs);

/* Starts rf clock */
extern __func_tab__ void (*ble_phy_rfclk_enable)(void);

/* Stops rf clock */
extern __func_tab__ void (*ble_phy_rfclk_disable)(void);

/*
 * Used to restart reception on same channel after wfr timer expiration or
 * frame received.
 */
extern __func_tab__ void (*ble_phy_restart_rx)(void);

/* Gets the current state of the PHY */
extern __func_tab__ int (*ble_phy_state_get)(void);

/* Gets current state of transceiver */
extern __func_tab__ uint8_t (*ble_phy_xcvr_state_get)(void);

/* Returns 'true' if a reception has started */
extern __func_tab__ int (*ble_phy_rx_started)(void);
 ...
 */
extern __func_tab__ uint8_t (*ble_phy_max_data_pdu_pyld)(void);

/* Gets the current access address */
extern __func_tab__ uint32_t (*ble_phy_access_addr_get)(void);

/* Enable encryption */
extern __func_tab__ void (*ble_phy_encrypt_enable)(uint64_t pkt_counter, uint8_t *iv, uint8_t *key,
                            uint8_t is_central);

/* Disable encryption */
extern __func_tab__ void (*ble_phy_encrypt_disable)(void);

/* Set the packet counters and dir used by LE encyption */
extern __func_tab__ void (*ble_phy_encrypt_set_pkt_cntr)(uint64_t pkt_counter, int dir);

/* Enable phy resolving list */
extern __func_tab__ void (*ble_phy_resolv_list_enable)(void);

/* Disable phy resolving list */
extern __func_tab__ void (*ble_phy_resolv_list_disable)(void);

/* Get resolving list enabled status */
extern __func_tab__ uint8_t (*ble_phy_resolv_list_enabled)(void);
...
extern __func_tab__ uint32_t (*ble_phy_mode_pdu_start_off)(int phy);
extern __func_tab__ void (*ble_phy_mode_set)(uint8_t tx_phy_mode, uint8_t rx_phy_mode);
#else
#define ble_phy_mode_pdu_start_off(phy)     (40)

#endif

extern __func_tab__ int (*ble_phy_get_cur_phy)(void);
...
extern __func_tab__ void(*ble_phy_enable_dtm)(void);
extern __func_tab__ void(*ble_phy_disable_dtm)(void);
#endif

#if (SCM2010)
extern __func_tab__ uint8_t * (*ble_phy_get_conn_ind_buf)(void);
extern __func_tab__ int (*ble_phy_tx_conn_ind)(ble_phy_tx_pducb_t pducb, void *pducb_arg, uint8_t end_trans);
#endif
