...
union ble_ll_conn_sm_flags {
    struct {
        ...
        uint32_t peer_phy_update: 1; /* XXX:combine with ctrlr update bit? */
...
struct ble_ll_conn_sm
{
    ...
    uint8_t last_rxd_sn;        /* note: can't be 1 bit given current code */
    ...
    /* XXX: do we need ce_end_time? Can't this be sched end time? */
...
#if (SCM2010)
    STAILQ_HEAD(conn_pend_txq_head, os_mbuf_pkthdr) conn_pend_txq;
#endif
...
__romfunc__ extern void (*ble_ll_conn_get_anchor)(struct ble_ll_conn_sm *connsm, uint16_t conn_event,
                            uint32_t *anchor, uint8_t *anchor_usecs);

#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
__romfunc__ extern int (*ble_ll_conn_move_anchor)(struct ble_ll_conn_sm *connsm, uint16_t offset);
#endif

struct ble_ll_scan_addr_data;
struct ble_ll_scan_pdu_data;

__romfunc__ extern uint8_t (*ble_ll_conn_tx_connect_ind_pducb)(uint8_t *dptr, void *pducb_arg,
                                         uint8_t *hdr_byte);
...
__romfunc__ extern int (*ble_ll_conn_send_connect_req)(struct os_mbuf *rxpdu,
                                 struct ble_ll_scan_addr_data *addrd,
                                 uint8_t ext);
...
#if (SCM2010)
extern __func_tab__ void (*ble_ll_conn_module_deinit)(void);
#endif
