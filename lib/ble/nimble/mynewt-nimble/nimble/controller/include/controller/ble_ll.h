...
#if (SCM2010)
    /* public generation event */
    struct ble_npl_event ll_p256_pubky_ev;

    /* dhkey generation event */
    struct ble_npl_event ll_dhkey_ev;

    /* local private key */
    uint8_t ll_priv_key[32];

    /* local public key*/
    uint8_t ll_pub_key[64];

    /* peer public key*/
    uint8_t ll_peer_pub_key[64];
#endif
};
...
__romfunc__ extern int (*ble_ll_reset)(void);
 ...
 *         NULL on failure (oom)
 */
__romfunc__ extern struct os_mbuf * (*ble_ll_rxpdu_alloc)(uint16_t len);
...
__romfunc__ extern int (*ble_ll_rx_start)(uint8_t *rxbuf, uint8_t chan, struct ble_mbuf_hdr *hdr);

/* Called by the PHY when a packet reception ends */
__romfunc__ extern int (*ble_ll_rx_end)(uint8_t *rxbuf, struct ble_mbuf_hdr *rxhdr);
...
/* Set random address */
__romfunc__ extern int (*ble_ll_set_random_addr)(const uint8_t *cmdbuf, uint8_t len, bool hci_adv_ext);

/* Wait for response timer expiration callback */
__romfunc__ extern void (*ble_ll_wfr_timer_exp)(void *arg);

/* Read set of features supported by the Link Layer */
__romfunc__ extern uint64_t (*ble_ll_read_supp_features)(void);

/* Set host supported features */
__romfunc__ extern int (*ble_ll_set_host_feat)(const uint8_t *cmdbuf, uint8_t len);

/* Read set of states supported by the Link Layer */
__romfunc__ extern uint64_t (*ble_ll_read_supp_states)(void);
...
extern __func_tab__ int (*ble_ll_rx_end_preprocess)(uint8_t *rxbuf, struct ble_mbuf_hdr *rxdr,
        uint8_t crcokd, uint8_t *processed);

extern __func_tab__ int (*ble_ll_deinit)(void);
extern void patch_ble_ll_supp_feature(void);
#endif
