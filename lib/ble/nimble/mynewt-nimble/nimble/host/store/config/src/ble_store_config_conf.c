...
#if SCM2010
    int fd = os_open(setting_name, O_CREAT | O_RDWR);
    if (fd < 0) {
        return BLE_HS_ESTORE_FAIL;
    }
    os_write(fd, buf, sizeof(buf));
    os_close(fd);
#else
...
ble_store_config_persist_cccds(void)
{
    ...
    ble_store_config_serialize_arr(ble_store_config_cccds,
                                   ...
                                   sizeof buf);
#if SCM2010
    int fd = os_open("/ble_hs/cccd", O_CREAT | O_RDWR);
    if (fd < 0) {
        return BLE_HS_ESTORE_FAIL;
    }
    os_write(fd, buf, sizeof(buf));
...
{
#if SCM2010
    char buf[BLE_STORE_CONFIG_SEC_SET_ENCODE_SZ > BLE_STORE_CONFIG_CCCD_SET_ENCODE_SZ ?
             BLE_STORE_CONFIG_SEC_SET_ENCODE_SZ : BLE_STORE_CONFIG_CCCD_SET_ENCODE_SZ];
    int fd;

    fd = os_open("/ble_hs/our_sec", O_RDWR);
    if (fd >= 0) {
        os_read(fd, buf, sizeof(buf));
        os_close(fd);
    ...
    fd = os_open("/ble_hs/peer_sec", O_RDWR);
    if (fd >= 0) {
        os_read(fd, buf, sizeof(buf));
        os_close(fd);
        ble_store_config_deserialize_arr(
