                     ...
                     ble_att_svr_access_fn *cb, void *cb_arg)
{
	...
	if (STAILQ_EMPTY(&ble_att_svr_list)) {
		STAILQ_INSERT_TAIL(&ble_att_svr_list, entry, ha_next);
	} else {
		struct ble_att_svr_entry *ha;
		struct ble_att_svr_entry *prev = NULL;
		uint8_t inserted = 0;

		STAILQ_FOREACH(ha, &ble_att_svr_list, ha_next) {
			if (entry->ha_handle_id < ha->ha_handle_id) {
				if (prev) {
					STAILQ_INSERT_AFTER(&ble_att_svr_list, prev, entry, ha_next);
				} else {
					STAILQ_INSERT_HEAD(&ble_att_svr_list, entry, ha_next);
				}
    ...
    if (author) {
		...
		if (entry->ha_cb) {
		    rc = entry->ha_cb(conn_handle, entry->ha_handle_id,
		                      is_read ? BLE_ATT_ACCESS_OP_READ : BLE_ATT_ACCESS_OP_WRITE, 0, NULL, entry->ha_cb_arg);
		    if (rc != 0) {
				*out_att_err = rc;
				return BLE_HS_ATT_ERR(*out_att_err);
			}
...
int
ble_att_svr_check_notify_perms(uint16_t conn_handle, uint16_t attr_handle,
                        uint8_t *out_att_err)
{
    struct ble_att_svr_entry *entry;
    struct ble_gap_sec_state sec_state;
    ...
    if ((enc || authen) && !sec_state.encrypted) {
        ...
        rc = ble_store_read_peer_sec(&key_sec, &value_sec);
        if (rc == 0 && value_sec.ltk_present) {
            att_err = BLE_ATT_ERR_INSUFFICIENT_ENC;
        } else {
            att_err = BLE_ATT_ERR_INSUFFICIENT_AUTHEN;
        }
        goto err;
    }
    ...
    if (entry->ha_min_key_size > sec_state.key_size) {
        att_err = BLE_ATT_ERR_INSUFFICIENT_KEY_SZ;
        goto err;
    }
    return 0;
...
#ifdef CONFIG_NIMBLE_PTS
void
ble_att_svr_set_svc_handle(uint16_t handle)
{
    if (handle) {
        ble_att_svr_id = handle - 1;
    }
}
#endif
