...
#define BLE_GAP_UPDATE_TIMEOUT_MS               5000 /* ms */
#else
...
#ifdef CONFIG_NIMBLE_PTS
static int
ble_gap_rx_adv_report_sanity_check_pts(const uint8_t *adv_data, uint8_t adv_data_len, struct ble_gap_disc_desc *desc)
{
	...
	if (ble_gap_master.disc.observation) {
		return 0;
	}
	...
	if ((rc == 0) && (flags->length == 2)) {
		if (ble_gap_master.disc.limited &&
		   !(flags->value[0] & BLE_HS_ADV_F_DISC_LTD)) {
			goto ignore;
		} else if (!ble_gap_master.disc.limited &&
		   !(flags->value[0] & (BLE_HS_ADV_F_DISC_LTD | BLE_HS_ADV_F_DISC_GEN))) {
			goto ignore;
		}

		memcpy(&ble_gap_master.disc.addr, &desc->addr, sizeof(ble_addr_t));
	} else {
		if (desc->event_type != BLE_HCI_ADV_RPT_EVTYPE_SCAN_IND) {
			goto ignore;
		}

		if (ble_addr_cmp(&ble_gap_master.disc.addr, &desc->addr)) {
			goto ignore;
		}

		memset(&ble_gap_master.disc.addr, 0, sizeof(ble_addr_t));
	}
...
#if NIMBLE_BLE_SCAN
#ifdef CONFIG_NIMBLE_PTS
    if (ble_gap_rx_adv_report_sanity_check_pts(desc->data, desc->length_data, desc)) {
#else
...
#ifdef CONFIG_NIMBLE_PTS
    ble_gap_master.disc.observation = params.observation;
	memset(&ble_gap_master.disc.addr, 0, sizeof(ble_addr_t));
#endif
    ble_gap_master.cb = cb;
    ble_gap_master.cb_arg = cb_arg;
