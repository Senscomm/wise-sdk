...
#define BLE_STORE_MAX_BONDED MYNEWT_VAL(BLE_STORE_MAX_BONDS)
#define BLE_STORE_MAX_CCCDS  MYNEWT_VAL(BLE_STORE_MAX_CCCDS)

ble_addr_t g_stored_key[BLE_STORE_MAX_BONDED];
struct ble_store_key_cccd g_stored_cccd[BLE_STORE_MAX_CCCDS];
int g_stored_key_idx;
int g_stored_cccd_idx;
...
#if SCM2010
static int
ble_store_keystore_iterator(int obj_type,
		                    union ble_store_value *val,
							void *cookie)
{
	int need_delete = 1;
    ...
    switch (obj_type) {
        case BLE_STORE_OBJ_TYPE_PEER_SEC:
			memcpy(&g_stored_key[g_stored_key_idx++], &val->sec.peer_addr, sizeof(ble_addr_t));

			break;
        case BLE_STORE_OBJ_TYPE_CCCD:
			for (i = 0; i < g_stored_key_idx; i++) {
				if (ble_addr_cmp(&val->cccd.peer_addr, &g_stored_key[i]) == 0) {
					need_delete = 0;
				}
			}

			if (need_delete) {
				ble_store_key_from_value_cccd(&g_stored_cccd[g_stored_cccd_idx++], &val->cccd);
			}
...
void
ble_store_check_validation(void)
{
	int i;

	memset(g_stored_key, 0, sizeof(ble_addr_t) * BLE_STORE_MAX_BONDED);
	memset(g_stored_cccd, 0, sizeof(struct ble_store_key_cccd) * BLE_STORE_MAX_CCCDS);
	g_stored_key_idx = 0;
	g_stored_cccd_idx = 0;

	ble_store_iterate(BLE_STORE_OBJ_TYPE_PEER_SEC, &ble_store_keystore_iterator, NULL);

	ble_store_iterate(BLE_STORE_OBJ_TYPE_CCCD, &ble_store_keystore_iterator, NULL);

	for (i = 0; i < g_stored_cccd_idx; i++) {
		ble_store_delete_cccd(&g_stored_cccd[i]);
	}
}
#endif
