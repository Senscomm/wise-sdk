...
static void
ble_l2cap_reject_reason(struct ble_l2cap_sig_hdr *hdr, struct os_mbuf **om)
{
	uint16_t reason;
	int rc;
	...
	memcpy(&reason, (*om)->om_data, 2);

	switch (reason) {
	case BLE_L2CAP_SIG_ERR_CMD_NOT_UNDERSTOOD:
		printf("Reject: Command Not Understood\n");
		break;
	case BLE_L2CAP_SIG_ERR_MTU_EXCEEDED:
		printf("Reject: MTU Exceeded\n");
		break;
	case BLE_L2CAP_SIG_ERR_INVALID_CID:
		printf("Reject: Invalid CID\n");
		break;
	default:
		printf("Uknown Reject Reason %d\n", reason);
		break;
	}
}
#endif
...
#ifdef CONFIG_NIMBLE_PTS
   ble_l2cap_reject_reason(hdr, om);
#endif
...
#ifdef CONFIG_NIMBLE_PTS
           ble_l2cap_sig_coc_connect_cb(proc, BLE_HS_EREJECT_EXT);
#else
