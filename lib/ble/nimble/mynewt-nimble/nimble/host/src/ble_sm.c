                   ...
                   struct ble_sm_result *res)
{
    ...
    if (proc != NULL) {
        ...
        conn = ble_hs_conn_find_assert(proc->conn_handle);
        /* First case is an AYLA ADDITION.
         */
        if (ble_hs_cfg.sm_no_pairing) {
            /* Fail the pairing request because pairing is currently disabled.
             */
            BLE_HS_LOG(WARN, "pairing disabled\n");
            res->sm_err = BLE_SM_ERR_PAIR_NOT_SUPP;
            res->app_status = BLE_HS_SM_US_ERR(BLE_SM_ERR_PAIR_NOT_SUPP);
                    ...
                    struct ble_sm_result *res)
{
    ...
    /*
     * Ayla workaround for ESP32-C3 bonding problem. The ID info message
     * (opcode BLE_SM_OP_IDENTITY_INFO 0x8) and ID addr info message
     * (opcode BLE_SM_OP_IDENTITY_ADDR_INFO 0x9) arrive while in state
     * BLE_SM_PROC_STATE_ENC_START rather than BLE_SM_PROC_STATE_KEY_EXCH
     * on the ESP32-C3 chip.
     *
     *  proc = ble_sm_proc_find(conn_handle, BLE_SM_PROC_STATE_KEY_EXCH, -1, NULL);
     */
    proc = ble_sm_proc_find(conn_handle, BLE_SM_PROC_STATE_NONE, -1, NULL);
