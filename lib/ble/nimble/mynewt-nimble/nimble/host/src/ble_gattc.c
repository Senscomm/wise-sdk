                                    ...
                                    struct os_mbuf **om)
{
    ...
    *om = NULL;
	...
	printf("Include Service: attr-hdl=0x%04x, start-hdl=0x%04x, end-hdl=0x%04x, uuid=%s\n",
			proc->find_inc_svcs.prev_handle, service.start_handle, service.end_handle, buf);
#endif
                                 ...
                                 struct ble_att_read_type_adata *adata)
{
    ...
    if (call_cb) {
		...
		printf("Include Service: attr-hdl=0x%04x, start-hdl=0x%04x, end-hdl=0x%04x, uuid=%s\n",
				adata->att_handle, service.start_handle, service.end_handle, buf);
                        ...
                        struct os_mbuf *txom)
{
    ...
    if (txom == NULL) {
        ...
        rc = ble_att_svr_check_notify_perms(conn_handle, chr_val_handle,
                                            NULL);
        if (rc != 0) {
                goto done;
        }
                          ...
                          struct os_mbuf *txom)
{
    ...
    if (txom == NULL) {
        ...
        rc = ble_att_svr_read_handle(conn_handle, chr_val_handle,
                                     0, txom, NULL);
        if (rc != 0) {
            goto done;
        }
