...
#if SCM2010
    /* try removing this entry first, because the controller may reject it
     * when it already has the same entry, disallowing the updates
     */
...
#ifdef CONFIG_NIMBLE_PTS
    rc = ble_hs_pvcy_set_addr_timeout(5);
#else
...
#ifdef CONFIG_NIMBLE_PTS
        /*
         * SCM2010 modification:
         * This seems to be needed to generate RPA when there is no bonding info
         * because the controller cannot generate an RPA when there is no IRK at all.
         * If such a feature is still needed, we can generate the RPA from
         * the application space, and provide it as our RANDOM address.
         */
