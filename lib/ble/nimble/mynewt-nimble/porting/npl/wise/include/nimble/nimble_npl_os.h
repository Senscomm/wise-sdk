...
{
    /* KJW - why ble_npl_eventq but not ble_npl_event?
     * Maybe okay because it is a pointer anyway.
     */
...
static inline bool IS_IRQ_MASKED(void)
{
	unsigned char epc = __nds__read_csr(NDS_MEPC);
	unsigned char mie = (__nds__read_csr(NDS_MSTATUS) & MSTATUS_MIE);
	/* mie is also cleared right after reset */
	return !mie && (epc != 0);
}
...
static int ble_npl_task_init(struct ble_npl_task *t, const char *name, ble_npl_task_func_t func,
         void *arg, uint8_t prio, ble_npl_time_t sanity_itvl,
         ble_npl_stack_t *stack_bottom, uint16_t stack_size)
{
    return npl_freertos_task_init(t, name, func, arg, prio, sanity_itvl, stack_bottom, stack_size);
}

static int ble_npl_task_remove(struct ble_npl_task *t)
{
    return npl_freertos_task_remove(t);
}
