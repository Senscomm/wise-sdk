...
#if defined(SCM2010) /* old port? */
#define vPortEnterCritical vTaskEnterCritical
#define vPortExitCritical vTaskExitCritical
#endif
                     ...
                     ble_npl_event_fn *ev_cb, void *ev_arg)
{
    ...
    /*
     * xTimerCreate function allocate timer handle from freertos heap.
     * But we don't find timer handle free routine.
     */
...
npl_freertos_hw_enter_critical(void)
{
    ...
    if (in_isr()) {
        uint32_t ctx;
        ctx = portSET_INTERRUPT_MASK_FROM_ISR();
        return ctx;
    } else {
        vPortEnterCritical();
        return 0;
...
#if 0
    if (in_isr()) {
        portCLEAR_INTERRUPT_MASK_FROM_ISR(ctx);
    } else {
        vPortExitCritical();
    }
#else
...
int npl_freertos_task_init(struct ble_npl_task *t, const char *name, ble_npl_task_func_t func,
...
int npl_freertos_task_remove(struct ble_npl_task *t)
{
