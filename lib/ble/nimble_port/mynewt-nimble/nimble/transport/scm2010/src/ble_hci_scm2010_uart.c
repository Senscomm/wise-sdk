...
ble_hci_uart_config(void)
{
    ...
    rc = hal_uart_init(0, NULL);
    if (rc != 0) {
        return BLE_ERR_UNSPECIFIED;
    }

    rc = hal_uart_init_cbs(0,
...
void
ble_hci_scm2010_uart_rx_handler(void *arg)
{
    (void) arg;
    while (1) {
        hal_uart_start_rx(0);
 ...
 */
void
ble_hci_scm2010_uart_init(void)
{
    int rc;
    ...
    printk("ble hci uart init\n");
}

void
ble_hci_scm2010_uart_deinit(void)
{
    hal_uart_close(0);
}
