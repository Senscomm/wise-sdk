...
/**
 * There is much room for improvement about this implementation.
 */
...
CYGACC_COMM_IF_GETC_TIMEOUT (char chan, char *c)
{
	...
	ch = getchar_timeout(xyzModem_CHAR_TIMEOUT);
	if (ch < 0)
		return 0;
...
zm_dump (int line)
{
	zm_dprintf ("Packet at line: %d (len=%d)\n", line, zm_bp - zm_buf);
...
static int xyzModem_get_hdr(void)
{
	...
	/* Verify checksum/CRC */
	if (xyz->crc_mode) {
		cksum = crc16_ccitt(xyz->pkt, xyz->len);
...
{
#if CONFIG_FLASH_DEBUG_VERBOSITY == 2
	switch (err) {
...
#elif CONFIG_FLASH_DEBUG_VERBOSITY == 1
	return "xyzModem error\n";
#else
	return "";
#endif
}
