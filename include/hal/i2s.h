...
#define IOCTL_I2S_SET_CONFIG        (1)
#define IOCTL_I2S_GET_CONFIG        (2)
#define IOCTL_I2S_TRIGGER           (3)
#define IOCTL_I2S_BLOCK_READ        (4)
#define IOCTL_I2S_BLOCK_WRITE       (5)
#define IOCTL_I2S_GET_STATE         (6)
...
enum i2s_state {
	...
	I2S_STATE_READY,
	/** The interface is started, but doesn't have date yet. */
    /** This is to support 'delayed start'. */
	I2S_STATE_STARTING,
...
enum i2s_trigger_cmd {
	 ...
	 * the i2s_buf_write() function. This trigger can be used in READY state
...
struct i2s_state_arg {
	enum i2s_dir dir;
	enum i2s_state state;
};

struct i2s_trigger_arg {
	enum i2s_dir dir;
	enum i2s_trigger_cmd cmd;
};

struct i2s_block_rw_arg {
	void *buf;
	size_t size;
};
...
struct i2s_ops {
	...
	const struct i2s_config *(*config_get)(struct device *dev, enum i2s_dir dir);
	int (*buf_read)(struct device *dev, void **mem_block, size_t *size);
	int (*buf_write)(struct device *dev, void *mem_block, size_t size);
	int (*trigger)(struct device *dev, enum i2s_dir dir, enum i2s_trigger_cmd cmd);
	int (*state_get)(struct device *dev, enum i2s_dir dir, enum i2s_state *state);
};

#define i2s_ops(x)  ((struct i2s_ops *)(x)->driver->ops)
 ...
 */
static __inline__ int i2s_configure(struct device *dev, enum i2s_dir dir,
			    const struct i2s_config *cfg)
{
	if (!dev)
		return -ENODEV;

	return i2s_ops(dev)->configure(dev, dir, cfg);
}
 ...
 */
static __inline__ const struct i2s_config *i2s_config_get(struct device *dev,
						      enum i2s_dir dir)
{
	if (!dev)
		return NULL;

	return i2s_ops(dev)->config_get(dev, dir);
}

/**
 * @brief Fetch state of a host I2S controller
 *
 * @param dev Pointer to the device structure for the driver instance
 * @param dir Stream direction: RX or TX as defined by I2S_DIR_*
 * @param state Get I2S state as defined by I2S_STATE_*
 * @retval 0 If successful.
 * @retval -EINVAL Invalid argument.
 * @retval -ENOSYS I2S_DIR_BOTH value is not supported.
 */
static __inline__ int i2s_state_get(struct device *dev,
						      enum i2s_dir dir, enum i2s_state *state)
{
	if (!dev)
		return -ENODEV;;

	return i2s_ops(dev)->state_get(dev, dir, state);
}
 ...
 */
static __inline__ int i2s_buf_read(struct device *dev, void **mem_block,
				 size_t *size)
{
	if (!dev)
		return -ENODEV;

	return i2s_ops(dev)->buf_read(dev, mem_block, size);
}
 ...
 * @retval -EAGAIN Waiting period timed out.
 */
static inline int i2s_buf_write(struct device *dev, void *mem_block,
			    size_t size)
{
	if (!dev)
		return -ENODEV;

	return i2s_ops(dev)->buf_write(dev, mem_block, size);
}
				     ...
				     enum i2s_trigger_cmd cmd)
{
	...
	return i2s_ops(dev)->trigger(dev, dir, cmd);
}
