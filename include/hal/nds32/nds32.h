...
#ifdef CONFIG_OPT_IFC
#define __TARGET_IFC_EXT
#endif
#endif

/* Support ZOL */
#ifdef CONFIG_OPT_HWZOL
...
#ifdef __WISE__
/* For some reason, GIE_RESTORE() is not inlined */
#define GIE_SAVE(var)			   \
{					   \
...
{
	if (old)
		*old = (isr_t)OS_CPU_Vector_Table[vector];

	OS_CPU_Vector_Table[vector] = (void *)isr;
}
