...
__STATIC_FORCEINLINE void L1C_CleanDCacheMVA(void *va) {
  ...
  __DSB();     //ensure the ordering of data cache maintenance operations and their effects
}
...
//Sect_Normal. Outer & inner wt, non-shareable, executable, rw, domain 0
#define section_normal_wt(descriptor_l1, region)     region.rg_t = SECTION; \
                                   region.domain = 0x0; \
                                   region.e_t = ECC_DISABLED; \
                                   region.g_t = GLOBAL; \
                                   region.inner_norm_t = WT; \
                                   region.outer_norm_t = WT; \
                                   region.mem_t = NORMAL; \
                                   region.sec_t = SECURE; \
                                   region.xn_t = EXECUTE; \
                                   region.priv_t = RW; \
