...
get_context(const char *node, const char *port) {
...
#if LWIP_IPV6
  const char *ip_any_addr = "::";
#else
  const char *ip_any_addr = "0.0.0.0";
#endif
  ...
  info_list = coap_resolve_address_info(node ? &local : coap_make_str_const(ip_any_addr), u_s_port, s_port,
                                              ws_port, wss_port,
...
static int
coap_getopt(int argc, char *argv[], const char *opts) {
  static int sp = 1;
  int c;
  char *cp;
  ...
  if (sp == 1) {
    if (optidx >= argc ||
        argv[optidx][0] != '-' || argv[optidx][1] == '\0') {
      return EOF;
        	}
    else if (!strcmp(argv[optidx], "--")) {
      optidx++;
      return EOF;
    }
  }
  optopt = c = argv[optidx][sp];
  ...
  if (*++cp == ':') {
    if (argv[optidx][sp+1] != '\0')
      optarg = &argv[optidx++][sp+1];
    else if (++optidx >= argc) {
...
static void coap_example_server(void       *p)
{
  ...
  struct param_t *param = (struct param_t *)p;
  ...
  while ((opt = coap_getopt(param->argc, param->argv,
    ...
    case 'i':
      if (!cmdline_read_identity_check(optarg)) {
        usage(param->argv[0], LIBCOAP_PACKAGE_VERSION);
        goto failed;
      }
      break;
    case 'j' :
...
finish:
  ...
  /* please note we SHOULD NOT cleanup the coap resource here,
  * since coap is a global variable and for our demo, both client
  * and server coexist
  */
#if 0
  coap_cleanup();
  ...
  printf("coap server exit by %s\n", (exit_code == 0) ? "command":"error");
...
static osThreadId_t example_tid;

int coap_cli_server_start(int argc, char *argv[])
{
	osThreadAttr_t attr = {
		.name = "coap_server",
		.stack_size = 6 * 1024,
	...
	quit = 0;
	example_tid = osThreadNew(coap_example_server, &param, &attr);

	return 0;
}

int coap_cli_server_stop(int argc, char *argv[])
{
	quit = 1;
	example_tid = NULL;
	return 0;
