...
static uint8_t cmd_buf[CMD_BUF_SZ];
...
cmd_gatt_service_changed(int argc, char **argv)
{
	...
	/* ble_svc_gatt_changed function only works if internal gatt service is used
	 * hard-coding for the large db 1
	 */
	ble_gatts_chr_updated(0x0012);
#else
	uint16_t start;
	uint16_t end;
	int rc;
	...
	while (1) {
		...
		if (is_long && offset) {
			if (bytes) {
				...
				for (i = 0; i < bytes; i++) {
					off_buf[offset + i] = (uint8_t)i;
				}

				attrs[num_attrs].om = ble_hs_mbuf_from_flat(off_buf,
						bytes + offset);
			} else {
				off_buf = calloc(1, attr_len + offset);

				if (!off_buf) {
					return -ENOSPC;
				}

				memcpy(off_buf + offset, cmd_buf + total_attr_len, attr_len);
			...
			if (bytes) {
				...
				attrs[num_attrs].om = ble_hs_mbuf_from_flat(buf, bytes);
...
#ifdef CONFIG_NIMBLE_PTS
int
cmd_gatt_svr(int argc, char **argv)
{
	int rc;
	uint8_t db;
	...
	db = parse_arg_uint8("db", &rc);
	if (rc != 0) {
		console_printf("invalid 'db' parameter\n");
		return rc;
	}

	switch (db) {
		case 0:
			gatt_svr_init_small_db();
		break;
		case 1:
			gatt_svr_init_large_db1();
		break;
		default:
		return -1;
	}
