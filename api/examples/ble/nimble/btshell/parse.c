...
{
#if CONFIG_NIMBLE_PTS
	if (strcmp(name, "conn") == 0) {
		uint16_t conn_handle;
		conn_handle = parse_arg_long_bounds(name, 0, UINT16_MAX, out_status);
		if (*out_status != 0) {
			extern int btshell_get_first_conn(uint16_t *conn_handle);
			int rc;

			rc = btshell_get_first_conn(&conn_handle);
			if (rc == 0) {
				*out_status = 0;
			} else {
		        *out_status = ENOENT;
...
int
parse_pts_oob_sc(char *name, uint8_t *dst, int out_len)
{
	char *sval;
	char byte[3] = {0, 0, 0};
	int i;
	...
	for (i = 0; i < out_len; i++) {
		memcpy(byte, &sval[i * 2], 2);
		dst[i] = strtol(byte, NULL, 16);
	}

	parse_reverse_bytes(dst, 16);
