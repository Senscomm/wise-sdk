...
#ifdef CONFIG_NET
#if (CONFIG_FLASH_IMAGE_SIZE < 0x180000)
#error Please use scm1612s_4m_defconfig or disable CONFIG_NET.
#endif
#endif

#if MYNEWT_VAL(BLE_ROLE_CENTRAL)
#define BTSHELL_MAX_SVCS               256
#define BTSHELL_MAX_CHRS               256
#define BTSHELL_MAX_DSCS               256
#else
...
static os_membuf_t btshell_svc_mem[OS_MEMPOOL_SIZE(BTSHELL_MAX_SVCS,
		sizeof(struct btshell_svc))];
...
#if MYNEWT_VAL(BLE_L2CAP_COC_MAX_NUM)
static os_membuf_t btshell_coc_conn_mem[ OS_MEMPOOL_SIZE(MYNEWT_VAL(BLE_L2CAP_COC_MAX_NUM),
...
/* This is to measure scanning performance with
 * assumption that the advertiser will include
 * a counter, which cycles in [00...FF], in the first
 * byte of complete local name.
 */
/* XXX: enable TEST_ADV_COUNTS in XH controller
 * if XH is to act as a counterpart.
 */
/* #define TEST_ADV_SCAN */

#ifdef TEST_ADV_SCAN
static uint32_t initial, last;
static uint32_t cycle, total, recvd;
#endif
...
uint8_t g_scan_report_verbose = 0;
uint8_t g_bt_shell_mem_enabled = 0;
		...
		const struct ble_gatt_error *error)
{
	...
	uint16_t att_err;
	const char *err_str = btshell_err_str(error->status, &att_err);
	console_printf("[PTS] att error = 0x%02x (%s)\n", att_err, err_str);
#endif
}
		...
		const struct ble_gatt_chr *gatt_chr)
{
	...
	if (svc == NULL) {
#ifdef CONFIG_NIMBLE_PTS
		printf("Characteristic: def-hdl=0x%04x, val-hdl=0x%04x, prop=%02x, uuid=",
				gatt_chr->def_handle, gatt_chr->val_handle, gatt_chr->properties);
		print_uuid(&gatt_chr->uuid.u);
		printf("\n");
#else
		...
		const struct ble_gatt_dsc *gatt_dsc)
{
	...
	if (svc == NULL) {
#ifdef CONFIG_NIMBLE_PTS
		printf("Descriptor: attr-hdl=0x%04x, uuid=", gatt_dsc->handle);
		print_uuid(&gatt_dsc->uuid.u);
		printf("\n");
#else
	...
	switch (error->status) {
		case 0:
			console_printf("characteristic read; conn_handle=%d "
					"attr_handle=%d offset=%d len=%d value=", conn_handle,
					attr->handle, attr->offset, OS_MBUF_PKTLEN(attr->om));
			print_mbuf(attr->om);
			console_printf("\n");
			break;
...
static int
btshell_check_name_filter(const uint8_t *adv_data, uint8_t adv_data_len, void *arg)
{
	...
	if (scan_opts && scan_opts->name_filter_len) {
		...
		if (ble_hs_adv_find_field(BLE_HS_ADV_TYPE_COMP_NAME, adv_data,
					adv_data_len, &ad_name)) {
			ble_hs_adv_find_field(BLE_HS_ADV_TYPE_INCOMP_NAME, adv_data,
					adv_data_len, &ad_name);
		}
...
#ifdef TEST_ADV_SCAN
    {
        uint32_t count = (uint32_t)adv_data[2];
        if (last == -1) {
            initial = count;
            /*
            printf("initial: %d\n", initial);
            ...
            */
             ...
             */
            cycle += 1;
            total = cycle * 256 - initial;
            console_printf("    total=%d, recvd=%d, reception=%d%%\n",
                    total, recvd, (recvd * 100) / total);
        }
        recvd++;
        last = count;
    }
...
static int
btshell_display_passkey(uint16_t conn_handle)
{
	struct ble_sm_io pk;
	int rc;
	...
	console_printf("The passkey notify number: 123456\n");

	return rc;
}

static int
btshell_oob_sc(uint16_t conn_handle)
{
	struct ble_sm_io pk;
	int rc;
...
{
	struct btshell_scan_opts *scan_opts;
	...
	switch (event->type) {
...
#ifdef CONFIG_NIMBLE_PTS
			console_printf("[PTS] connection %s\n", btshell_err_str(event->connect.status, NULL));
#endif
			return 0;
...
#ifdef CONFIG_NIMBLE_PTS
			console_printf("[PTS] disconnected; %s\n", btshell_err_str(event->disconnect.reason, NULL));
			gatt_svr_large_db1_author(0);
#endif
		...
		case BLE_GAP_EVENT_DISC:
			scan_opts = arg;
			if (ble_addr_cmp(&scan_opts->peer, BLE_ADDR_ANY)) {
				if ((event->disc.addr.type & 0x01) != scan_opts->peer.type ||
				   memcmp(event->disc.addr.val, scan_opts->peer.val, 6)) {
					return 0;
				}
			}

			if (btshell_check_name_filter(event->disc.data, event->disc.length_data, arg) < 0) {
				return 0;
			}

#ifndef TEST_ADV_SCAN
...
#ifdef CONFIG_NIMBLE_PTS
			if (scan_opts->raw_data_display) {
				int i;

				console_printf("RAW:");

				for (i = 0; i < event->disc.length_data; i++) {
					console_printf("%02x", event->disc.data[i]);
				}
...
#ifdef CONFIG_NIMBLE_PTS
			if (event->conn_update_req.peer_params->itvl_min == 8 &&
				event->conn_update_req.peer_params->itvl_max == 170 &&
				event->conn_update_req.peer_params->latency == 0 &&
				event->conn_update_req.peer_params->supervision_timeout == 2048) {
				return -1;
			}
#endif
		...
		case BLE_GAP_EVENT_PASSKEY_ACTION:
			...
			if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
				btshell_display_passkey(event->passkey.conn_handle);
			}

			if (event->passkey.params.action == BLE_SM_IOACT_OOB_SC) {
				btshell_oob_sc(event->passkey.conn_handle);
			}
			return 0;
...
#ifdef CONFIG_NIMBLE_PTS
			uint16_t smp_err;
			const char *err_str = btshell_err_str(event->enc_change.status, &smp_err);
			console_printf("\n[PTS] smp error = 0x%02x (%s)\n", smp_err, err_str);

			if (event->enc_change.status == 0) {
				gatt_svr_large_db1_author(1);
			}

			if (event->enc_change.status != 0) {
				if (smp_err == 0x06) { //PIN key missing
					console_printf("[PTS] PIN key missing - Bond lost\n");
...
#ifdef CONFIG_NIMBLE_PTS
			uint16_t att_err;
			console_printf("[PTS] status=0x%02x (%s)\n",
					event->notify_tx.status,
					btshell_err_str(event->notify_tx.status, &att_err));
#endif
			return 0;
		...
		case BLE_GAP_EVENT_REPEAT_PAIRING:
			console_printf("[PTS] Repeated pairing - Bond lost\n");
...
btshell_on_sync(void)
{
	...
	if (fd > 0) {
		console_printf("random address stored\n");
		...
		console_printf("random address generated\n");
		fd = os_open("/ble_hs/rand_addr", O_CREAT | O_WRONLY);
		if (fd > 0) {
			console_printf("random address saved\n");
...
#ifdef CONFIG_NIMBLE_PTS
	ble_addr_t pts_addr;
	fd = os_open("/ble_hs/pts_addr", O_RDONLY);
	if (fd > 0) {
		os_read(fd, &pts_addr, sizeof(ble_addr_t));
		console_printf("pts address: addr_type=%s addr=",
				pts_addr.type == BLE_ADDR_RANDOM ? "random" : "public");
		print_addr(&pts_addr.val);
		console_printf("\n");
	} else {
		console_printf("pts address not configured\n");
	}
#endif
	...
	console_printf("random addr ");
	print_addr(addr.val);
	console_printf("\n");

	ble_hs_id_copy_addr(BLE_ADDR_PUBLIC, addr.val, NULL);

	console_printf("public addr ");
	print_addr(addr.val);
	console_printf("\n");
...
#ifdef CONFIG_NIMBLE_PTS
	btshell_l2cap_create_srv(0x27, 0, 0);
	btshell_l2cap_create_srv(0xf2, 0, BLE_HS_EAUTHEN);
	btshell_l2cap_create_srv(0xf3, 0, BLE_HS_EAUTHOR);
	btshell_l2cap_create_srv(0xf4, 0, BLE_HS_EENCRYPT_KEY_SZ);
	btshell_l2cap_create_srv(0xf5, 0, BLE_HS_EENCRYPT);

	struct ble_hs_adv_fields fields;
	bssnz_t ble_uuid16_t uuids16[2];

	memset(&fields, 0, sizeof fields);

	uuids16[0] = (ble_uuid16_t) BLE_UUID16_INIT(0x1800);
	uuids16[1] = (ble_uuid16_t) BLE_UUID16_INIT(0x1801);;

	fields.flags = BLE_HS_ADV_F_BREDR_UNSUP;
	fields.uuids16 = uuids16;
	fields.num_uuids16 = 2;
	fields.uuids16_is_complete = 1;
	fields.name = (uint8_t *)"PTS-GAP-06B8";
	fields.name_len = 12;
	fields.name_is_complete = 1;
	fields.appearance = 0x00;
...
#ifdef CONFIG_NIMBLE_PTS
	console_printf("CoC received data = ");
	print_mbuf(sdu);
	console_printf("\n");
#endif
...
#ifdef CONFIG_NIMBLE_PTS
				if (event->connect.status == BLE_HS_EREJECT_EXT) {
					return 0;
				}

				uint16_t coc_err = btshell_hs_err_to_coc_err(event->connect.status);
				const char *err_str = btshell_l2coc_err_str(coc_err);
				console_printf("[PTS] l2cap coc error: 0x%04x (%s)\n", coc_err, err_str);
#endif
				return 0;
			}
...
#ifdef CONFIG_NIMBLE_PTS
int
btshell_get_first_conn(uint16_t *conn_handle)
{
...
btshell_init(void)
{
	...
	if (!g_bt_shell_mem_enabled) {
		...
		g_bt_shell_mem_enabled = 1;
#endif
	}

	if (ble_hs_is_enabled()) {
		console_printf("already enabled\n");
		return 0;
	}
